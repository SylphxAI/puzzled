...
| Console **D-co** (736/734 of 857 `settings.json` values identical to `en-US` in the zh locales; `/settings/notifications` hardcoded English; one toggle with no accessible name) | **In flight** on `s2/console-copy`; anything not covered there stays enumerated in the PR body. |
| Core play **D-cp** (`games/*/translations/en.json` injected for all five locales) and Marketing **D2** (38 games' tips/FAQ English in every zh locale) | **In flight** on `s2/game-copy`: locale-aware selection with a declared English fallback, copy for the ritual's modules, and an explicit remaining list. |
| Auth **D3** (`/unsubscribe` untranslated) | **In flight** on `s2/console-copy`. |

...
- **#144** `perf(web): first-paint, bundle and third-party waste` — open, mergeable, five checks green, build still running.
...
Each stage names its **definition of done** and the **evidence layer** at which done is claimed (`Local` / `Candidate` / `Check-CI` / `Landed` / `Artifact` / `Released` / `Deployed` / `Live`, per `standards/proof.md`). "Merged" is never silently "done".
...
| **S4** | SEO, meta and structured data | Per-surface title/description/OG/Twitter/canonical/hreflang correct and unique; `Organization`/`WebSite` extended to page-level types where they earn a rich result; the existing `verify:seo` harness runs in CI and fails on drift | Check-CI + Deployed readback |
...
| S1 | Active | #144 mergeable, build running; #149 conflicting against `main` |

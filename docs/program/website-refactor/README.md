...
| S2 | Active — source work landed | The audit is complete and landed ([`s2-audit.md`](s2-audit.md)); 13 axes conform, 8 deviate (15 distinct defects), 4 unverifiable on the auditing host. All three defect families are fixed on `main`: home FAQ copy (#151, `878ca92`), per-locale game copy with a declared English fallback (#155, `1d5680a`), console + `/unsubscribe` copy and the unnamed switch (#157, `77a2d75`). Remaining: the catalog focus-ring deviation (owner = catalog surface), Chinese copy for 14 of 19 modules, the 360px coverage hole, and new register rows **G10** (the harnesses run but do not gate a merge) and **G11** (the performance gate is red before any change). |

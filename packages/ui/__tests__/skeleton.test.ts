...
describe('SkeletonTable column widths', () => {
	describe('header widths', () => {
		...
		test('cycles through widths for many columns', () => {
			const widths = Array.from({ length: 10 }, (_, i) => getColumnWidth(i, true))
...
describe('skeleton width calculations', () => {
	test('last line width for text is reduced', () => {
		...
		expect(parseInt(lastLineWidth)).toBeLessThan(parseInt(otherLineWidth))
	})
	...
	test('breadcrumb last item is wider', () => {
		...
		const lastWidth = parseInt(lastItemWidth.replace('w-', ''))
		const otherWidth = parseInt(otherItemWidth.replace('w-', ''))

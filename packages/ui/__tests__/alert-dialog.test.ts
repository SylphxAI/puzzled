...
function getAlertDialogOverlayClasses(className?: string) {
	return cn('alert-dialog-overlay fixed inset-0 z-modal bg-black/50', className)
}
...
function getAlertDialogActionClasses(variant: 'default' | 'destructive' = 'default', className?: string) {
...
describe('ConfirmDialog icon styling', () => {
	...
	function getIconClasses(variant: 'default' | 'destructive') {
		return cn('h-5 w-5', variant === 'destructive' ? 'text-destructive' : 'text-warning')
	}

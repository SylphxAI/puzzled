...
function getSheetPanelClasses(side: SheetSide = 'right', className?: string) {
	...
	return cn('sheet-panel fixed z-drawer bg-background shadow-lg', sideClasses[side], className)
}
...
function getSheetFooterClasses(className?: string) {
	return cn('flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 p-4 border-t border-border', className)
}

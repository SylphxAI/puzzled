...
function calculatePercentage(value: number, max: number = 100): number {
	return Math.min(Math.max((value / max) * 100, 0), 100)
}

function getProgressClasses(size: ProgressSize = 'md') {
	return cn('relative w-full overflow-hidden rounded-full bg-muted', sizeStyles[size])
}

function getIndicatorClasses(variant: ProgressVariant = 'default') {
	return cn('h-full rounded-full transition-all duration-500', variantStyles[variant])
}

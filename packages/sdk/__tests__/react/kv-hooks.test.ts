...
describe('Rate limit result handling', () => {
	...
	test('checks if rate limited', () => {
		...
		expect(isRateLimited({ success: true, limit: 100, remaining: 50, reset: 0 })).toBe(false)
		expect(isRateLimited({ success: false, limit: 100, remaining: 0, reset: 0 })).toBe(true)
	})
...
describe('Sorted set member handling', () => {
	...
	test('gets top N members', () => {
		function getTopN(members: KvZMember[], n: number): KvZMember[] {
			return [...members].sort((a, b) => (b.score ?? 0) - (a.score ?? 0)).slice(0, n)
		}
...
describe('Error handling', () => {
	test('extracts error message from response', () => {
		...
		expect(extractErrorMessage({ error: 'Key not found' })).toBe('Key not found')
	...
	test('creates error from API response', () => {
		function createApiError(response: { error?: string }): Error {
			const message = typeof response.error === 'string' ? response.error : 'Request failed'
...
describe('Hash operations', () => {
	...
	test('parses hgetall response', () => {
		function parseHgetallResponse<T>(response: { fields: Record<string, T> | null }) {
...
describe('Multi-key operations', () => {
	...
	test('parses mget response', () => {
		function parseMgetResponse<T>(response: { values: Record<string, T | null> }) {

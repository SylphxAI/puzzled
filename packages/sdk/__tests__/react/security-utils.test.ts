...
import { isValidRedirectUrl, safeRedirect, sanitizeUrl } from '../../src/react/security-utils'
...
describe('isValidRedirectUrl', () => {
	...
	describe('relative URLs', () => {
		...
		test('rejects relative paths when allowRelative is false', () => {
			expect(isValidRedirectUrl('/dashboard', { allowRelative: false })).toBe(false)
			expect(isValidRedirectUrl('/user/settings', { allowRelative: false })).toBe(false)
		})
	})
	...
	describe('same-origin URLs', () => {
		test('allows same-origin URLs', () => {
			expect(
				isValidRedirectUrl('https://example.com/path', { origin: 'https://example.com' })
			).toBe(true)
		})

		test('rejects cross-origin URLs without allowlist', () => {
			expect(
				isValidRedirectUrl('https://other.com/path', { origin: 'https://example.com' })
			).toBe(false)
		})
		...
		test('handles port differences', () => {
			expect(
				isValidRedirectUrl('https://example.com:8080/path', { origin: 'https://example.com' })
			).toBe(false)
			expect(
		...
		test('handles protocol differences', () => {
			expect(
				isValidRedirectUrl('http://example.com/path', { origin: 'https://example.com' })
			).toBe(false)
		})
	})
	...
	describe('protocol validation', () => {
		test('allows http: protocol', () => {
			expect(
				isValidRedirectUrl('http://example.com/path', { origin: 'http://example.com' })
			).toBe(true)
		})
		...
		test('rejects non-http(s) protocols', () => {
			expect(
				isValidRedirectUrl('ftp://example.com/file', { origin: 'https://example.com' })
			).toBe(false)
			expect(
				isValidRedirectUrl('mailto:user@example.com', { origin: 'https://example.com' })
			).toBe(false)
			expect(
				isValidRedirectUrl('tel:+1234567890', { origin: 'https://example.com' })
			).toBe(false)
		})
	})
	...
	describe('malformed URLs', () => {
		test('rejects invalid URLs that cannot be parsed', () => {
			expect(
				isValidRedirectUrl('not-a-url-at-all::::', { origin: 'https://example.com' })
			).toBe(false)
		})
...
describe('Real-World Attack Scenarios', () => {
	...
	describe('Open redirect attacks', () => {
		test('blocks direct external redirects', () => {
			expect(
				isValidRedirectUrl('https://evil.com/phishing', { origin: 'https://myapp.com' })
			).toBe(false)
		})

...
import { createWebhookHandler, verifyWebhook } from "../src/server/index";
...
describe("verifyWebhook", () => {
	...
	describe("valid signatures", () => {
		test("verifies valid signature from header", async () => {
			const { body, signatureHeader } = await createSignedWebhook(
				testPayload,
				testSecret,
			);
	...
	describe("signature validation", () => {
		...
		test("rejects wrong signature", async () => {
			const { body, timestamp } = await createSignedWebhook(
				testPayload,
				testSecret,
			);
		...
		test("rejects tampered payload", async () => {
			const { signatureHeader } = await createSignedWebhook(
				testPayload,
				testSecret,
			);

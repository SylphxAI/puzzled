...
describe("updateOrganization", () => {
	test("updates organization name", async () => {
		...
		const result = await updateOrganization(mockConfig, "org-123", {
			name: "Updated Name",
		});
...
describe("canManageMembers", () => {
	...
	test("returns false for viewer role", () => {
		expect(canManageMembers({ role: "viewer" } as OrganizationMembership)).toBe(
			false,
		);
	});
	...
	test("returns false for developer role", () => {
		expect(
			canManageMembers({ role: "developer" } as OrganizationMembership),
		).toBe(false);
	});

	test("returns false for billing role", () => {
		expect(
			canManageMembers({ role: "billing" } as OrganizationMembership),
		).toBe(false);
	});

	test("returns true for admin role", () => {
		expect(canManageMembers({ role: "admin" } as OrganizationMembership)).toBe(
			true,
		);
	});
	...
	test("returns true for super_admin role", () => {
		expect(
			canManageMembers({ role: "super_admin" } as OrganizationMembership),
		).toBe(true);
	});
});
...
describe("canManageSettings", () => {
	...
	test("returns false for non-admin roles", () => {
		expect(
			canManageSettings({ role: "viewer" } as OrganizationMembership),
		).toBe(false);
		expect(
			canManageSettings({ role: "developer" } as OrganizationMembership),
		).toBe(false);
		expect(
			canManageSettings({ role: "billing" } as OrganizationMembership),
		).toBe(false);
	});

	test("returns true for admin and super_admin", () => {
		expect(canManageSettings({ role: "admin" } as OrganizationMembership)).toBe(
			true,
		);
		expect(
			canManageSettings({ role: "super_admin" } as OrganizationMembership),
		).toBe(true);
	});
});
...
describe("canDeleteOrganization", () => {
	...
	test("returns false for non-super_admin roles", () => {
		expect(
			canDeleteOrganization({ role: "viewer" } as OrganizationMembership),
		).toBe(false);
		expect(
			canDeleteOrganization({ role: "developer" } as OrganizationMembership),
		).toBe(false);
		expect(
			canDeleteOrganization({ role: "admin" } as OrganizationMembership),
		).toBe(false);
	});

	test("returns true only for super_admin", () => {
		expect(
			canDeleteOrganization({ role: "super_admin" } as OrganizationMembership),
		).toBe(true);
	});
});
...
describe("hasRole edge cases", () => {
	test("returns false for undefined membership", () => {
		expect(
			hasRole(undefined as unknown as OrganizationMembership | null, "viewer"),
		).toBe(false);
	});
...
describe("Role boundary conditions", () => {
	...
	test("only super_admin can access super_admin", () => {
		...
		expect(
			hasRole({ role: "super_admin" } as OrganizationMembership, "super_admin"),
		).toBe(true);
	});
});
...
describe("API error handling", () => {
	...
	test("handles 403 forbidden response", async () => {
		...
		await expect(
			updateOrganization(mockConfig, "org-123", { name: "New" }),
		).rejects.toThrow();
	});

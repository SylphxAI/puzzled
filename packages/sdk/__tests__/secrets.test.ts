...
describe('getSecret', () => {
	...
	test('handles secret not found error', async () => {
		...
		await expect(getSecret(mockConfig, { key: 'NONEXISTENT' })).rejects.toThrow()
	})
})
...
describe('listSecretKeys', () => {
	...
	test('filters by environment ID via query parameter', async () => {
		const mockResponse = [
			{ key: 'PROD_KEY', description: null, version: 'v1', isEnvironmentSpecific: true },
		]
		globalThis.fetch = mock((url: string, options: RequestInit) => {
			fetchCalls.push({ url, options })
			return Promise.resolve(createMockResponse(mockResponse))
		...
		const result = await listSecretKeys(mockConfig, { environmentId: 'env-production' })
...
describe('hasSecret', () => {
	test('returns true when secret exists', async () => {
		const mockResponse = [
			{ key: 'DATABASE_URL', description: null, version: 'v1', isEnvironmentSpecific: false },
			{ key: 'API_KEY', description: null, version: 'v1', isEnvironmentSpecific: false },
		]
		globalThis.fetch = mock((url: string, options: RequestInit) => {
			fetchCalls.push({ url, options })
			return Promise.resolve(createMockResponse(mockResponse))
	...
	test('returns false when secret does not exist', async () => {
		const mockResponse = [
			{ key: 'OTHER_KEY', description: null, version: 'v1', isEnvironmentSpecific: false },
		]
		globalThis.fetch = mock((url: string, options: RequestInit) => {
			fetchCalls.push({ url, options })
			return Promise.resolve(createMockResponse(mockResponse))
...
describe('getAllSecrets', () => {
	...
	test('passes environment ID to both calls', async () => {
		...
		globalThis.fetch = mock((url: string, options: RequestInit) => {
			...
			// Second call: getSecrets
			return Promise.resolve(createMockResponse({ PROD_KEY: 'production-value' }))
		}) as typeof fetch
...
describe('Edge Cases', () => {
	...
	test('handles secret values with special characters', async () => {
		...
		expect(result.value).toBe('postgres://user:p@ss=w0rd!@host:5432/db?ssl=true&param=val')
	})
	...
	test('handles null description in secret keys', async () => {
		const mockResponse = [
			{ key: 'NO_DESC_KEY', description: null, version: 'v1', isEnvironmentSpecific: false },
		]
		globalThis.fetch = mock((url: string, options: RequestInit) => {
			fetchCalls.push({ url, options })
			return Promise.resolve(createMockResponse(mockResponse))

...
describe('getConsentTypes', () => {
	test('fetches consent types from correct endpoint', async () => {
		...
		expect(url).toContain('/api/v1/consent/types')
	})
...
describe('hasConsent', () => {
	test('calls correct endpoint', async () => {
		...
		expect(mockFetch).toHaveBeenCalledTimes(1)
		const url = getRequestUrl()
		expect(url).toContain('/api/v1/consent/check')
	})
	...
	test('returns true when consent is granted', async () => {
		...
		const result = await hasConsent(testConfig, 'analytics', {
			userId: 'user-123',
		})
	...
	test('returns false when consent is not granted', async () => {
		...
		const result = await hasConsent(testConfig, 'marketing', {
			userId: 'user-123',
		})
...
describe('getUserConsents', () => {
	test('fetches user consents from correct endpoint', async () => {
		...
		expect(mockFetch).toHaveBeenCalledTimes(1)
		const url = getRequestUrl()
		expect(url).toContain('/api/v1/consent/user')
	})
...
describe('setConsents', () => {
	test('posts to correct endpoint', async () => {
		...
		expect(mockFetch).toHaveBeenCalledTimes(1)
		const url = getRequestUrl()
		expect(url).toContain('/api/v1/consent/set')
	})
...
describe('acceptAllConsents', () => {
	test('posts to correct endpoint', async () => {
		...
		expect(mockFetch).toHaveBeenCalledTimes(1)
		const url = getRequestUrl()
		expect(url).toContain('/api/v1/consent/accept-all')
	})
...
describe('declineOptionalConsents', () => {
	test('posts to correct endpoint', async () => {
		...
		expect(mockFetch).toHaveBeenCalledTimes(1)
		const url = getRequestUrl()
		expect(url).toContain('/api/v1/consent/decline-optional')
	})
...
describe('linkAnonymousConsents', () => {
	test('posts to correct endpoint', async () => {
		...
		expect(mockFetch).toHaveBeenCalledTimes(1)
		const url = getRequestUrl()
		expect(url).toContain('/api/v1/consent/link-anonymous')
	})

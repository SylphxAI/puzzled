...
describe('cancelJob', () => {
	...
	test('handles already running job', async () => {
		globalThis.fetch = mock((url: string, options: RequestInit) => {
			fetchCalls.push({ url, options })
			return Promise.resolve(createMockResponse({ success: false, reason: 'Job is already running' }))
		}) as typeof fetch
...
describe('listJobs', () => {
	...
	test('filters jobs by status', async () => {
		const mockResponse = {
			jobs: [{ id: 'job_1', status: 'pending', createdAt: '2025-01-15T09:00:00Z' }],
			total: 1,
		}
		globalThis.fetch = mock((url: string, options: RequestInit) => {
			fetchCalls.push({ url, options })
	...
	test('paginates with limit and offset', async () => {
		const mockResponse = {
			jobs: [{ id: 'job_11', status: 'completed', createdAt: '2025-01-15T09:00:00Z' }],
			total: 25,
		}
		globalThis.fetch = mock((url: string, options: RequestInit) => {
			fetchCalls.push({ url, options })
...
describe('createCron', () => {
	...
	test('includes all cron options in request', async () => {
		const mockResponse = { scheduleId: 'sched_123', cron: '*/5 * * * *', paused: true }
		globalThis.fetch = mock((url: string, options: RequestInit) => {
			fetchCalls.push({ url, options })
			return Promise.resolve(createMockResponse(mockResponse))
		}) as typeof fetch
	...
	test('creates cron with idempotency key', async () => {
		const mockResponse = { scheduleId: 'sched_123', cron: '0 0 * * 0', paused: false }
		globalThis.fetch = mock((url: string, options: RequestInit) => {
			fetchCalls.push({ url, options })
			return Promise.resolve(createMockResponse(mockResponse))
		}) as typeof fetch
	...
	test('creates cron in paused state', async () => {
		const mockResponse = { scheduleId: 'sched_paused', cron: '0 0 1 * *', paused: true }
		globalThis.fetch = mock((url: string, options: RequestInit) => {
			fetchCalls.push({ url, options })
			return Promise.resolve(createMockResponse(mockResponse))
		}) as typeof fetch

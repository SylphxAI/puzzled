...
describe("createCheckout", () => {
	...
	test("sends correct request body", async () => {
		...
		globalThis.fetch = async (_url, init) => {
			...
			return new Response(
				JSON.stringify({ checkoutUrl: "https://checkout.stripe.com/xyz" }),
			);
		};
	...
	test("supports all intervals", async () => {
		const intervals: Array<"monthly" | "annual" | "lifetime"> = [
			"monthly",
			"annual",
			"lifetime",
		];
...
describe("createPortalSession", () => {
	...
	test("sends correct request body", async () => {
		...
		globalThis.fetch = async (_url, init) => {
			capturedBody = init?.body as string;
			return new Response(
				JSON.stringify({ portalUrl: "https://billing.stripe.com/xyz" }),
			);
		};

...
describe("refreshToken", () => {
	...
	test("throws on invalid refresh token", async () => {
		...
		await expect(refreshToken(config, "invalid-token")).rejects.toThrow(
...
describe("verifyEmail", () => {
	test("completes successfully", async () => {
		...
		const config = createTestConfig();
		await expect(
			verifyEmail(config, "verify-token-123"),
...
describe("forgotPassword", () => {
	test("completes successfully", async () => {
		...
		const config = createTestConfig();
		await expect(
			forgotPassword(config, "user@example.com"),
		).resolves.toBeUndefined();
	});
...
describe("verifyTwoFactor", () => {
	...
	test("throws on invalid code", async () => {
		...
		await expect(verifyTwoFactor(config, "user-123", "000000")).rejects.toThrow(
			"Invalid 2FA code",
		);
	});

...
describe("getAllStreaks", () => {
	...
	test("returns array of streak states", async () => {
		const streaks = [
			mockStreakState,
			{ ...mockStreakState, streakId: "weekly-quest" },
		];
		mockFetch.mockImplementationOnce(() =>
			Promise.resolve(
...
describe("recoverStreak", () => {
	test("posts to correct endpoint", async () => {
		mockFetch.mockImplementationOnce(() =>
			Promise.resolve(
				new Response(
					JSON.stringify({ success: true, streak: mockStreakState }),
					{
						status: 200,
						headers: { "Content-Type": "application/json" },
					},
...
describe("getUserLeaderboardRank", () => {
	...
	test("returns rank and value", async () => {
		...
		const result = await getUserLeaderboardRank(
			testConfig,
			"high-scores",
			"user-123",
		);
...
describe("incrementAchievementProgress", () => {
	test("posts to correct endpoint", async () => {
		mockFetch.mockImplementationOnce(() =>
			Promise.resolve(
				new Response(
					JSON.stringify({ ...mockUserAchievement, progress: 50, target: 100 }),
					{
						status: 200,
						headers: { "Content-Type": "application/json" },
					},
...
describe("Error Handling", () => {
	...
	test("leaderboard functions throw on network error", async () => {
		...
		await expect(
			getLeaderboard(testConfig, "test", "user-123"),
		).rejects.toThrow();
	...
	test("achievement functions throw on network error", async () => {
		...
		await expect(
			unlockAchievement(testConfig, "test", "user-123"),
		).rejects.toThrow();
	});
	...
	test("throws on 404 response", async () => {
		...
		await expect(
			getStreak(testConfig, "nonexistent", "user-123"),
		).rejects.toThrow();
	});
});

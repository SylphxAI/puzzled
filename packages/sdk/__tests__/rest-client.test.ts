...
import { type RestClientConfig, createRestApi } from "../src/react/rest-client";
...
describe("GET requests", () => {
	test("makes GET request to correct URL", async () => {
		...
		expect(getLastCall()?.url).toBe(
			"https://sylphx.com/api/sdk/v1/user/profile",
		);
...
describe("POST requests", () => {
	test("makes POST request with body", async () => {
		...
		const result = await api.post<{ id: string }>("/items", {
			name: "New Item",
		});
...
describe("PUT requests", () => {
	test("makes PUT request with body", async () => {
		...
		const result = await api.put<{ updated: boolean }>("/items/123", {
			name: "Updated",
		});
...
describe("401 Recovery", () => {
	...
	test("does not retry 401 twice (prevents infinite loop)", async () => {
		...
		globalThis.fetch = mock((url: string, options: RequestInit) => {
			...
			return Promise.resolve(
				new Response(
					JSON.stringify({ error: { message: "Still unauthorized" } }),
					{
						status: 401,
						headers: { "Content-Type": "application/json" },
					},

...
describe('page', () => {
	test('sends page view to correct endpoint', async () => {
		...
		expect(mockFetch).toHaveBeenCalledTimes(1)
		const url = getRequestUrl()
		expect(url).toContain('/api/v1/analytics/page')
	})
...
describe('identify', () => {
	test('sends identify to correct endpoint', async () => {
		...
		expect(mockFetch).toHaveBeenCalledTimes(1)
		const url = getRequestUrl()
		expect(url).toContain('/api/v1/analytics/identify')
	})
...
describe('trackBatch', () => {
	test('sends batch to correct endpoint', async () => {
		...
		expect(mockFetch).toHaveBeenCalledTimes(1)
		const url = getRequestUrl()
		expect(url).toContain('/api/v1/analytics/batch')
	})
...
describe('Edge Cases', () => {
	...
	test('handles array properties', async () => {
		...
		const body = getRequestBody()
		expect((body.properties as Record<string, unknown>).items).toEqual(['item1', 'item2', 'item3'])
	})

...
describe("trackBatch", () => {
	...
	test("transforms page events to $pageview", async () => {
		...
		expect((events[0].properties as Record<string, unknown>).name).toBe(
			"Home Page",
		);
	...
	test("transforms identify events to $identify", async () => {
		...
		expect(
			(
				(events[0].properties as Record<string, unknown>).traits as Record<
					string,
					unknown
				>
			).email,
...
describe("Error Handling", () => {
	...
	test("trackBatch throws on error", async () => {
		...
		await expect(
			trackBatch(testConfig, [{ type: "track", event: "test" }]),
		).rejects.toThrow();
	});
});

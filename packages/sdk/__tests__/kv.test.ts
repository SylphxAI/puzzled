...
import { type KvClient, createKv, getKv } from "../src/server/kv";
...
describe("ratelimit", () => {
	test("checks rate limit", async () => {
		...
		const result = await kv.ratelimit("api:user123", {
			limit: 100,
			window: "1h",
		});
...
describe("zrange", () => {
	...
	test("gets range with scores reversed", async () => {
		...
		const result = await kv.zrange("leaderboard", 0, 9, {
			withScores: true,
			rev: true,
		});
...
describe("getKv", () => {
	...
	test("throws when secret key not configured", () => {
		...
		try {
			...
			expect(() => createKv({ platformUrl: "https://test.com" })).toThrow(

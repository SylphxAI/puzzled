...
describe('verifyPKCE', () => {
	describe('S256 method', () => {
		...
		test('returns false for tampered challenge', async () => {
			...
			const result = await verifyPKCE(codeVerifier, 'tampered-challenge', 'S256')
			expect(result).toBe(false)
		})
...
describe('PKCE integration', () => {
	test('full OAuth flow simulation', async () => {
		...
		const isValid = await verifyPKCE(codeVerifier, codeChallenge, codeChallengeMethod)
		expect(isValid).toBe(true)

		// Step 4: Attacker intercepts code but doesn't have verifier
		const attackerVerifier = generateCodeVerifier()
		const attackerValid = await verifyPKCE(attackerVerifier, codeChallenge, codeChallengeMethod)
	...
	test('multiple concurrent PKCE flows', async () => {
		...
		for (let i = 0; i < flows.length; i++) {
			for (let j = 0; j < flows.length; j++) {
				const result = await verifyPKCE(flows[i].codeVerifier, flows[j].codeChallenge)

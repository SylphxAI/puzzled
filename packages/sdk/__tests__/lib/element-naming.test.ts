...
function sanitizeText(text: string): string {
	return text.replace(/\s+/g, " ").trim().slice(0, 100).replace(/[<>]/g, "");
}
...
describe("sanitizeText", () => {
	...
	describe("HTML removal", () => {
		test("removes angle brackets", () => {
			expect(sanitizeText("hello <script>alert</script>")).toBe(
...
describe("element naming integration", () => {
	...
	test("form field name processing", () => {
		const fieldNames = ["items[]", "items[0]", "address_line_1", "phoneNumber"];
	...
	test("button text sanitization", () => {
		const texts = ["  Sign Up  ", "Submit\nForm", "Click <here> to continue"];

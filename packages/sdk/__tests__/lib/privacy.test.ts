...
import {
	sanitizeForLogging,
	sanitizeUrl,
} from "../../src/lib/monitoring/session-replay/privacy";
...
describe("sanitizeForLogging", () => {
	describe("email redaction", () => {
		...
		test("redacts multiple emails", () => {
			...
			expect(sanitizeForLogging(text)).toBe(
		...
		test("handles email at end of string", () => {
			expect(sanitizeForLogging("Contact test@example.com")).toBe(
...
describe("sanitizeUrl", () => {
	describe("token parameter", () => {
		test("redacts token parameter", () => {
			...
			expect(sanitizeUrl(url)).toBe(
				"https://example.com/api?token=%5BREDACTED%5D",
			);
		});
	});
	...
	describe("key parameter", () => {
		test("redacts key parameter", () => {
			...
			expect(sanitizeUrl(url)).toBe(
				"https://example.com/api?key=%5BREDACTED%5D",
			);
		});
	});
	...
	describe("secret parameter", () => {
		test("redacts secret parameter", () => {
			...
			expect(sanitizeUrl(url)).toBe(
				"https://example.com/oauth?secret=%5BREDACTED%5D",
			);
		});
	});
	...
	describe("auth parameter", () => {
		test("redacts auth parameter", () => {
			...
			expect(sanitizeUrl(url)).toBe(
				"https://example.com/api?auth=%5BREDACTED%5D",
			);
		});
	});
	...
	describe("api_key parameter", () => {
		test("redacts api_key parameter", () => {
			...
			expect(sanitizeUrl(url)).toBe(
				"https://example.com/v1?api_key=%5BREDACTED%5D",
			);
		});
	});
	...
	describe("apikey parameter", () => {
		test("redacts apikey parameter", () => {
			...
			expect(sanitizeUrl(url)).toBe(
				"https://example.com/v1?apikey=%5BREDACTED%5D",
			);
		});
	});
...
describe("privacy integration", () => {
	...
	test("sanitizes error message with PII", () => {
		...
		expect(sanitized).toBe(
			"Failed to send email to [EMAIL_REDACTED] from IP [IP_REDACTED]",
		);
	});

	test("sanitizes API callback URL", () => {
		const callbackUrl =
			"https://app.com/callback?code=auth_code_123&token=access_token";

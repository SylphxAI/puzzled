...
function isSensitiveByPattern(name: string, id: string, autocomplete: string): boolean {
...
function isClickableRole(role: string | null): boolean {
	...
	return ['button', 'link', 'tab', 'menuitem', 'option'].includes(role.toLowerCase())
}

function isMeaningfulClickTarget(tagName: string, inputType?: string, role?: string): boolean {
	...
	// Role-based
	if (role && ['button', 'link', 'tab', 'menuitem'].includes(role.toLowerCase())) {
		return true
	}
...
describe('autocapture integration', () => {
	...
	test('sensitive field detection for common patterns', () => {
		// Payment fields
		expect(isSensitiveByPattern('card_number', 'cardNum', 'cc-number')).toBe(true)
		expect(isSensitiveByPattern('', 'cvv', '')).toBe(true)

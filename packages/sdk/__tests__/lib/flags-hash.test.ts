...
import { murmurHash3, getBucket, isInPercentage, getUserBucket, selectVariant } from '../../src/lib/flags/hash'
...
describe('selectVariant', () => {
	...
	describe('weight normalization', () => {
		test('handles weights that dont sum to 100', () => {
			...
			const aCount = [0, 25, 49].filter((b) => selectVariant(variants, b) === 'a').length
			const bCount = [50, 75, 99].filter((b) => selectVariant(variants, b) === 'b').length

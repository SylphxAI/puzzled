...
import { DEFAULT_ANALYTICS_CONFIG, DEFAULT_AUTOCAPTURE_CONFIG } from '../../src/lib/analytics/types'
import type { AnalyticsEvent, EventProperties } from '../../src/lib/analytics/types'
...
describe('DEFAULT_AUTOCAPTURE_CONFIG', () => {
	test('has correct default values', () => {
		...
		expect(DEFAULT_AUTOCAPTURE_CONFIG.eventTypes).toEqual(['click', 'submit', 'change'])
	})
	...
	test('has correct default attributes', () => {
		...
		expect(DEFAULT_AUTOCAPTURE_CONFIG.captureAttributes).toContain('data-testid')
		expect(DEFAULT_AUTOCAPTURE_CONFIG.captureAttributes).toContain('data-analytics')
	})
})
...
describe('user properties', () => {
	test('setUserProperties tracks $set event', () => {
		...
		expect(events[0].properties.$set).toEqual({ name: 'Test User', plan: 'premium' })
	})
	...
	test('setUserPropertiesOnce tracks $set_once event', () => {
		...
		expect(events[0].properties.$set_once).toEqual({ first_seen: '2024-01-01' })
	})
...
describe('group', () => {
	test('group tracks $groupidentify event', () => {
		...
		tracker.group('company', 'acme-corp', { name: 'ACME Corp', plan: 'enterprise' })
		...
		expect(events[0].properties.$group_set).toEqual({ name: 'ACME Corp', plan: 'enterprise' })
	})

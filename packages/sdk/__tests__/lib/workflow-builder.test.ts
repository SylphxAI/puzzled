...
import type { StepContext, WorkflowDefinition, JobStep } from '../../src/lib/jobs/types'
...
describe('createWorkflow', () => {
	...
	describe('callbacks', () => {
		...
		test('sets onError callback', () => {
			const callback = (error: Error, step: string) => console.error(error, step)
			const workflow = createWorkflow('test')
				.onError(callback)
				.build()
...
describe('withRetry', () => {
	...
	test('adds retry delay', () => {
		const originalJob = job('send-email')
		const retriedJob = withRetry(originalJob, { maxRetries: 3, retryDelay: 1000 })

		expect(retriedJob.options?.retryDelay).toEqual({ type: 'fixed', delay: 1000 })
	})
...
describe('fanOut', () => {
	...
	test('sets maxConcurrency', () => {
		const step = fanOut([1, 2, 3], 'process', () => ({}), { maxConcurrency: 5 })
		expect(step.maxConcurrency).toBe(5)
	})
...
describe('validateWorkflow', () => {
	describe('basic validation', () => {
		...
		test('error for missing type', () => {
			const workflow = { steps: [job('test')] } as unknown as WorkflowDefinition
...
describe('workflow builder integration', () => {
	test('complex onboarding workflow', () => {
		const workflow = createWorkflow('user-onboarding')
			...
			.step('create-account', job('create-user', (ctx) => ({
	...
	test('saga pattern workflow', () => {
		...
		const workflow = createWorkflow('order-process')
			.steps(...orderSaga.map((step, i) => [`step-${i}`, step] as [string, typeof step]))
			.build()

...
import type { FlagDefinition, EvaluationContext, FlagVariant } from '../../src/lib/flags/types'
...
function createBooleanFlag(key: string, enabled: boolean, defaultValue: boolean = true): FlagDefinition {
	return {
		key,
		enabled,
	...
	describe('getJSON', () => {
		test('returns JSON variant value', () => {
			const flag: FlagDefinition = {
				...
				variants: [
					{ key: 'default', value: { maxItems: 10, theme: 'light' } },
				],
				rules: [],
			}
			evaluator.setFlags([flag])
...
describe('evaluator integration', () => {
	...
	test('debug mode logging', () => {
		...
		try {
			...
			expect(consoleLogs.some((log) => log.includes('[FeatureFlags]'))).toBe(true)
		} finally {

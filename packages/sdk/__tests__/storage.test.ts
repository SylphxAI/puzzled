...
import { deleteFile, type FileInfo, getFileInfo, getFileUrl } from '../src/storage'
...
describe('deleteFile', () => {
	...
	test('calls correct endpoint with file ID', async () => {
		...
		expect(capturedUrl).toContain('/api/v1/storage/files/file-xyz-456')
	...
	test('throws on file not found', async () => {
		...
		await expect(deleteFile(config, 'nonexistent')).rejects.toThrow('File not found')
	})
...
describe('getFileUrl', () => {
	...
	test('calls correct endpoint', async () => {
		...
		globalThis.fetch = async (url) => {
			capturedUrl = url.toString()
			return new Response(JSON.stringify({ url: 'https://cdn.example.com/file' }))
		}

		const config = createTestConfig()
		await getFileUrl(config, 'file-abc')

		expect(capturedUrl).toContain('/api/v1/storage/files/file-abc')
	})
	...
	test('throws on file not found', async () => {
		...
		await expect(getFileUrl(config, 'nonexistent')).rejects.toThrow('File not found')
	})
})
...
describe('getFileInfo', () => {
	...
	test('calls correct endpoint', async () => {
		...
		expect(capturedUrl).toContain('/api/v1/storage/files/file-xyz')
	})
	...
	test('throws on file not found', async () => {
		...
		await expect(getFileInfo(config, 'nonexistent')).rejects.toThrow('File not found')
	})

	test('handles various content types', async () => {
		const contentTypes = ['image/jpeg', 'image/png', 'application/pdf', 'text/plain', 'video/mp4']

...
import { hasError, getRestErrorMessage } from '../src/rest-client'
...
describe('hasError', () => {
	...
	describe('edge cases', () => {
		test('handles response with both data and error', () => {
			...
			const response = { data: { id: 1 }, error: { message: 'Partial failure' } }
			expect(hasError(response)).toBe(true)
		})
...
describe('getRestErrorMessage', () => {
	describe('extracts message from error object', () => {
		...
		test('extracts from error.error.message', () => {
			const error = { error: { message: 'Invalid input', code: 'VALIDATION_ERROR' } }
	...
	describe('handles unknown error types', () => {
		...
		test('returns default for string', () => {
			...
			expect(getRestErrorMessage('Some error')).toBe('An unknown error occurred')
		})
		...
		test('returns default for array', () => {
			expect(getRestErrorMessage(['error1', 'error2'])).toBe('An unknown error occurred')
		})
	})

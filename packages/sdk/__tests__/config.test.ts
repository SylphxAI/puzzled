...
describe("buildApiUrl", () => {
	test("builds correct URL with leading slash", () => {
		...
		expect(buildApiUrl(config, "/auth/login")).toBe(
			"https://sylphx.com/api/sdk/v1/auth/login",
		);
	});
...
describe("callApi", () => {
	...
	test("parses JSON response", async () => {
		globalThis.fetch = async () => {
			return new Response(
				JSON.stringify({ user: { id: "123", name: "Test" } }),
			);
		};

		const config = createConfig({ platformUrl: "https://example.com" });
		const result = await callApi<{ user: { id: string; name: string } }>(
			config,
			"/test",
		);

...
## ADR-007: Schema-First Type Generation
...
The SDK has 147+ manually defined types that are duplicates of Platform Zod schemas. When Platform changes a schema, SDK types don't automatically update, causing runtime errors and type mismatches.

### Decision

All SDK request/response types MUST be derived from `generated/api.d.ts` (generated by openapi-typescript from the Platform's OpenAPI spec). Manual type definitions for API types are **prohibited**.

### Pattern

```typescript
// ❌ PROHIBITED — Manual type definition
export interface SignInInput {
  email: string
  password: string
}

// ✅ REQUIRED — Import from generated types
...
|----------|--------|----------|
| **API Request/Response** | `generated/api.d.ts` | `LoginRequest`, `LoginResponse`, `Plan` |
| **SDK-Internal** | Manual definition | `SylphxConfig`, `UserCookieData`, `AppConfig` |
| **React-Specific** | Manual definition | `SylphxProviderProps`, `SdkUserProfile` |

### Naming Convention

Generated types use Platform schema names directly:
- `LoginRequest` (not `SignInInput`)
- `LoginResponse` (not `SignInResult`)
- `Plan` (not `BillingPlan`)

This ensures 1:1 mapping with API documentation.

### Workflow

```bash
# After Platform schema changes:
bun run generate:types:local   # Regenerate api.d.ts
bun run typecheck              # Catch breaking changes
```

### Consequences

**Positive:**
- Schema changes automatically propagate to SDK types
- TypeScript catches breaking changes at compile time
- Single Source of Truth (Platform Zod schemas)
- Types match API documentation exactly

**Negative:**
- Must run `generate:types` before types are available
- Breaking changes in Platform immediately break SDK builds (intentional)

### Migration

All 12 service modules migrated to generated types:
- `auth.ts`, `billing.ts`, `consent.ts`, `analytics.ts`
- `flags.ts`, `webhooks.ts`, `referrals.ts`, `jobs.ts`
- `storage.ts`, `notifications.ts`, `email.ts`, `ai.ts`

Reference implementation: `orgs.ts` (already using this pattern)
...
7. **Schema-first types** - all API types from generated OpenAPI spec

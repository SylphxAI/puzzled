...
export function createStorageValue(config: CreateStorageValueConfig): StorageContextValue {
	...
	const handleUpload = async (
		...
		const { presignedUrl, url, storageKey, tokenPayload } = await tokenRes.json() as PresignedTokenResponse
		...
		if (!completeRes.ok) {
			const err = await completeRes.json().catch(() => ({ error: 'Unknown error' })) as { error?: string }
	...
	return {
		...
		deleteFile: async (fileId: string) => {
			const res = await fetch(`${platformUrl}/api/storage/${fileId}`, { method: 'DELETE' })

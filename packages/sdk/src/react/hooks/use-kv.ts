...
import { DEFAULT_PLATFORM_URL, SDK_API_PATH, SDK_VERSION, SDK_PLATFORM, MAX_RETRIES, BASE_RETRY_DELAY_MS } from '../../constants'
...
export function useKv(options: UseKvOptions = {}): UseKvReturn {
	...
	const get = useCallback(async <T>(key: string): Promise<{ value: T | null; ttl: number | null }> => {
		return request<{ value: T | null; ttl: number | null }>('GET', `/${encodeURIComponent(key)}`)
	}, [request])

	const set = useCallback(async <T>(key: string, value: T, options?: KvSetOptions): Promise<boolean> => {
		const result = await request<{ success: boolean }>('POST', '', { key, value, ...options })
		return result.success
	}, [request])

	const del = useCallback(async (key: string): Promise<number> => {
	...
	const mget = useCallback(async <T>(keys: string[]): Promise<Record<string, T | null>> => {
		const result = await request<{ values: Record<string, T | null> }>('POST', '/mget', { keys })
		return result.values
	}, [request])

	const mset = useCallback(async <T>(
	...
	const incr = useCallback(async (key: string, by = 1): Promise<number> => {
		const result = await request<{ value: number }>('POST', '/incr', { key, by })
		return result.value
	}, [request])

	const expire = useCallback(async (key: string, seconds: number): Promise<boolean> => {
		const result = await request<{ success: boolean }>('POST', '/expire', { key, seconds })
		return result.success
	}, [request])

	const ratelimit = useCallback(async (key: string, options: { limit: number; window: string }): Promise<KvRateLimitResult> => {
		return request<KvRateLimitResult>('POST', '/ratelimit', { key, ...options })
	}, [request])

	const hset = useCallback(async <T>(key: string, fields: Record<string, T>): Promise<number> => {
		const result = await request<{ created: number }>('POST', '/hset', { key, fields })
		return result.created
	}, [request])

	const hget = useCallback(async <T>(key: string, field: string): Promise<T | null> => {
		const result = await request<{ value: T | null }>('POST', '/hget', { key, field })
		return result.value
	}, [request])

	const hgetall = useCallback(async <T>(key: string): Promise<Record<string, T> | null> => {
		const result = await request<{ fields: Record<string, T> | null }>('POST', '/hgetall', { key })
		return result.fields
	}, [request])

	const lpush = useCallback(async <T>(key: string, ...values: T[]): Promise<number> => {
		const result = await request<{ length: number }>('POST', '/lpush', { key, values })
		return result.length
	}, [request])

	const lrange = useCallback(async <T>(key: string, start = 0, stop = -1): Promise<T[]> => {
		const result = await request<{ values: T[] }>('POST', '/lrange', { key, start, stop })
		return result.values
	}, [request])

	const zadd = useCallback(async (key: string, ...members: Array<{ score: number; member: string }>): Promise<number> => {
		const result = await request<{ added: number }>('POST', '/zadd', { key, members })
		return result.added
	}, [request])

	const zrange = useCallback(async (
		key: string,
		start = 0,
		stop = 9,
	...
	return useMemo(() => ({
		...
		zadd,
		zrange,
	}), [get, set, del, exists, mget, mset, incr, expire, ratelimit, hset, hget, hgetall, lpush, lrange, zadd, zrange])
}

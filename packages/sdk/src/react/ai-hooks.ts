			...
			try {
				...
				if (options.stream) {
					...
					for await (const chunk of ctx.chatStream(chatInput)) {
						...
						const token =
							chunk.choices?.[0]?.delta?.content ?? chunk.delta?.content ?? "";
						if (token) {
							fullContent += token;
	...
	const complete = useCallback(
		async (prompt: string): Promise<string> => {
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Completion failed");
				setError(error);
				options.onError?.(error);
				throw error;
			} finally {
	...
	const embed = useCallback(
		async (text: string): Promise<number[]> => {
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Embedding failed");
				setError(error);
				throw error;
			} finally {
				setIsLoading(false);
	...
	const vision = useCallback(
		async (
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Vision analysis failed");
				setError(error);
				throw error;
			} finally {
				setIsLoading(false);

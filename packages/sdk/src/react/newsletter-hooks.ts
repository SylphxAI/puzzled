...
export interface UseNewsletterReturn {
	...
	getUnsubscribeInfo: (
		token: string,
	) => Promise<{ email: string; name?: string; isUnsubscribed: boolean }>;
	...
	updatePreferences: (
		...
		success: boolean;
		preferences: Array<{ key: SubscriberPreferenceKey; enabled: boolean }>;
	}>;
	...
	const verify = useCallback(
		async (token: string) => {
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Failed to verify");
				setError(error);
				throw error;
			} finally {
				setIsLoading(false);
		...
		async (token: string) => {
			try {
				...
				const error =
					err instanceof Error
						? err
						: new Error("Failed to get unsubscribe info");
				setError(error);
				throw error;
			}
		},
	...
	const getPreferences = useCallback(
		async (email: string) => {
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Failed to get preferences");
				setError(error);
				throw error;
			}
		},
...
export function useSubscriberForm(
	options: UseSubscriberFormOptions = {},
): UseSubscriberFormReturn {
	const ctx = useNewsletterContext();
	const [email, setEmail] = useState("");

...
export function SecuritySettings({
		...
		queryFn: async () => {
			...
			return data.map((s) => ({
				...
				location:
					s.city && s.country
						? `${s.city}, ${s.country}`
						: s.location || s.country || undefined,
	...
	const queryError =
		twoFactorQuery.error || sessionsQuery.error || loginHistoryQuery.error;
	useEffect(() => {
		if (queryError) {
			const message =
	...
	if (isLoading) {
		return (
			<div
				style={mergeStyles(styles.flexCenter, { padding: "3rem" })}
				className={className}
			>
				<span
			...
			{show2FA && (
				<div style={cardStyles}>
					<div style={styles.flexBetween}>
						...
						{twoFactor.enabled ? (
							<span style={mergeStyles(styles.badge, styles.badgeSuccess)}>
								Enabled
							</span>
						) : (
							<span
					...
					{showSetup2FA && totpQrCode && (
						...
						>
							<h5 style={{ margin: "0 0 0.5rem", fontWeight: 500 }}>
								Setup Instructions
							</h5>
							<ol
							...
							<div
								style={mergeStyles(styles.flexCenter, { marginBottom: "1rem" })}
							>
								<img
									src={totpQrCode}
									alt="2FA QR Code"
								...
								<div style={mergeStyles(styles.flexRow, { gap: "0.5rem" })}>
									<button
										...
										style={mergeStyles(
											...
											isEnabling2FA || verifyCode.length !== 6
...
function DeviceIcon({
	device,
	theme,
}: { device: string; theme: ThemeVariables }) {
	const isMobile =

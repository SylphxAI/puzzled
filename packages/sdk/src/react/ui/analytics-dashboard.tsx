	...
	return (
		<div style={containerStyle} className={className}>
			<div style={headerStyle}>
				<h3 style={{ margin: 0, fontSize: theme.fontSizeLg, fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
				...
				{isLoading ? (
					...
					filteredEvents.map((event, i) => (
						<div
							...
							style={{
								...eventItemStyle,
								borderBottom: i === filteredEvents.length - 1 ? 'none' : eventItemStyle.borderBottom,
							}}
						>
							<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
								<div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
									...
									/>
									<span style={{ fontWeight: 500, fontSize: theme.fontSizeSm }}>{event.name}</span>
								</div>
								{showTimestamps && (
							...
							{/* Expanded details */}
							{expandedId === event.id && event.properties && Object.keys(event.properties).length > 0 && (
								<div
									style={{
										marginTop: '0.75rem',
										padding: '0.75rem',
	...
	return (
		<div style={containerStyle} className={className}>
			<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
				<span style={{ fontSize: theme.fontSizeSm, color: theme.colorMutedForeground }}>{label}</span>
				{getIcon()}
			</div>
			...
			{isLoading ? (
				...
				<>
					<div style={{ fontSize: theme.fontSizeXl, fontWeight: 600, marginBottom: '0.5rem' }}>
						{formatValue(value)}
					</div>

					{change !== undefined && (
						<div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: theme.fontSizeXs }}>
							<span
								style={{
									color: change >= 0 ? theme.colorSuccess : theme.colorDestructive,
									fontWeight: 500,
								}}
							>
								{change >= 0 ? '↑' : '↓'} {Math.abs(change).toFixed(1)}%
							</span>
							<span style={{ color: theme.colorMutedForeground }}>{changeLabel}</span>
						</div>
					)}
				</>
			)}
		...
		<div style={containerStyle} className={className}>
			{title && (
				<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
					...
					{yAxisLabel && (
						<span style={{ fontSize: theme.fontSizeXs, color: theme.colorMutedForeground }}>{yAxisLabel}</span>
					)}
				</div>
			)}
			...
			{isLoading ? (
				...
				<div style={{ height, display: 'flex', alignItems: 'center', justifyContent: 'center', color: theme.colorMutedForeground }}>
					No data available
				</div>
			) : (
				<div style={{ height }}>
...
export function AnalyticsDashboard({
	...
	return (
		<div style={containerStyle} className={className}>
			{period && (
				<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
					<h2 style={{ margin: 0, fontSize: theme.fontSizeXl, fontWeight: 600 }}>Analytics</h2>
					<span style={{ fontSize: theme.fontSizeSm, color: theme.colorMutedForeground }}>{period}</span>
				</div>
			)}

			{stats.length > 0 && (
				<StatsGrid theme={theme} stats={stats} columns={Math.min(stats.length, 4) as 2 | 3 | 4} isLoading={isLoading} />
			)}

			{chartData.length > 0 && (
				<SimpleChart theme={theme} data={chartData} title={chartTitle} isLoading={isLoading} />
			)}

			{events.length > 0 && (
				<EventViewer theme={theme} events={events} isLoading={isLoading} maxEvents={10} />
			)}
		</div>
	)
}

...
import { type UseChatOptions, useChat } from "../ai-hooks";
...
export function ChatInterface({
	...
	const { messages, send, isLoading, error, clear, retry } =
		useChat(chatOptions);
	...
	const sendButtonStyle: CSSProperties = {
		...
		backgroundColor:
			isLoading || !input.trim() ? theme.colorMuted : theme.colorPrimary,
		color:
					...
					(renderEmpty?.() ?? (
						...
						>
							...
							<div>
								...
								<p
									style={{ margin: "0.25rem 0 0", fontSize: theme.fontSizeXs }}
								>
				...
				style={{
					...
					color:
						disabled || isLoading || !input.trim()
							...
							: theme.colorPrimaryForeground,
					cursor:
						disabled || isLoading || !input.trim() ? "not-allowed" : "pointer",
				}}
			>
				{isLoading ? (
					<span style={{ width: 16, height: 16 }} className="sylphx-spinner" />
				) : (

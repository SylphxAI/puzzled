	...
	return (
		<div style={containerStyle} className={className}>
			...
			<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
				<div>
					<h3 style={{ margin: 0, fontSize: theme.fontSizeLg, fontWeight: 600 }}>Webhooks</h3>
			...
			{/* Create Form */}
			{showCreate && (
				<div style={{ ...webhookCardStyle, backgroundColor: theme.colorMuted, marginBottom: '1.5rem' }}>
					<h4 style={{ margin: '0 0 1rem', fontSize: theme.fontSizeSm, fontWeight: 600 }}>New Webhook</h4>
					...
					<form onSubmit={handleCreate}>
								...
								{availableEvents.map((event) => (
									<button
										...
										style={{
											...
											backgroundColor: selectedEvents.includes(event) ? `${theme.colorPrimary}10` : 'transparent',
											color: selectedEvents.includes(event) ? theme.colorPrimary : theme.colorForeground,
											cursor: 'pointer',
										}}
									>
										{event}
						...
						<div style={{ display: 'flex', gap: '0.75rem' }}>
							...
							<button
								...
								style={mergeStyles(styles.button, styles.buttonPrimary)}
							>
								{isCreating ? <span style={styles.spinner} /> : 'Create Webhook'}
							</button>
						</div>
					</form>
				</div>
						...
						<div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
							<div style={{ flex: 1, minWidth: 0 }}>
								<div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
									<span
										style={{
											...
											height: 8,
											borderRadius: '50%',
											backgroundColor: webhook.enabled ? theme.colorSuccess : theme.colorMutedForeground,
										}}
									/>
									<span
										style={{
								...
								<div style={{ fontSize: theme.fontSizeXs, color: theme.colorMutedForeground }}>
									...
									{webhook.lastTriggeredAt && ` • Last triggered ${new Date(webhook.lastTriggeredAt).toLocaleString()}`}
									{webhook.failureCount !== undefined && webhook.failureCount > 0 && (
										<span style={{ color: theme.colorDestructive }}> • {webhook.failureCount} failures</span>
									)}
								</div>
							</div>
							<div style={{ display: 'flex', gap: '0.5rem', marginLeft: '1rem' }}>
								...
								{onTest && (
									...
									>
										{testingId === webhook.id ? <span style={styles.spinner} /> : 'Test'}
									</button>
								)}
								{onDelete && (
									<button
									...
									>
										{deletingId === webhook.id ? <span style={styles.spinner} /> : 'Delete'}
									</button>
								)}
							</div>
						</div>
...
export function WebhookDeliveryLog({
	...
	const statusColors: Record<WebhookDelivery['status'], { bg: string; text: string }> = {
					...
					displayedDeliveries.map((delivery, i) => (
						<div
							...
							style={{
								...
								borderBottom: i === displayedDeliveries.length - 1 ? 'none' : `1px solid ${theme.colorBorder}`,
								cursor: 'pointer',
							}}
						...
						>
							<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
								<div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
									...
									<span style={{ fontWeight: 500, fontSize: theme.fontSizeSm }}>{delivery.event}</span>
									{delivery.statusCode && (
								...
								>
									{delivery.payload && (
										<div style={{ marginBottom: '0.75rem' }}>
											<div style={{ fontSize: theme.fontSizeXs, fontWeight: 500, marginBottom: '0.25rem' }}>
												Payload
											</div>
											<pre style={{ margin: 0, fontSize: '0.625rem', fontFamily: 'monospace', whiteSpace: 'pre-wrap' }}>

...
export function CookieBanner({
	...
	const containerStyle: CSSProperties = mergeStyles(
		{
			...
			borderTop: position === 'bottom' || position === 'top' ? `1px solid ${theme.colorBorder}` : 'none',
			boxShadow: '0 -4px 20px rgba(0, 0, 0, 0.1)',
			transform: isVisible ? 'translateY(0)' : position.includes('top') ? 'translateY(-100%)' : 'translateY(100%)',
	...
	return (
		<div style={containerStyle} className={className} role="dialog" aria-label="Cookie consent">
			<div style={contentStyle}>
				{!showCustomize ? (
					// Main banner view
					<div style={variant === 'bar' ? mergeStyles(styles.flexBetween, { gap: '2rem', flexWrap: 'wrap' }) : {}}>
						<div style={{ flex: 1, minWidth: '200px' }}>
							<h3 style={{ margin: 0, fontSize: theme.fontSizeLg, fontWeight: 600, marginBottom: '0.5rem' }}>
								{title}
							</h3>
							<p style={mergeStyles(styles.textSm, styles.textMuted, { margin: 0, lineHeight: 1.6 })}>
								{description}
								{privacyPolicyUrl && (
									<>
										{' '}
						...
						<div style={mergeStyles(styles.flexRow, { gap: '0.75rem', flexWrap: 'wrap' })}>
					...
					<div>
						...
						<div style={{ marginBottom: '1.5rem' }}>
							{/* Required (always on) */}
							{types.filter(t => t.required).map(type => (
								<ConsentToggle
									key={type.id}
									name={type.name}
						...
						<div style={mergeStyles(styles.flexRow, { gap: '0.75rem', justifyContent: 'flex-end' })}>
							...
							<button
								...
								style={mergeStyles(styles.button, styles.buttonPrimary)}
							>
								{isLoading ? <span style={styles.spinner} /> : 'Save Preferences'}
							</button>
						</div>
					</div>
				)}
		...
		>
			<div style={{ flex: 1 }}>
				<div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
					<span style={{ fontWeight: 500, fontSize: theme.fontSizeSm }}>{name}</span>
					{required && (
						<span
							style={{

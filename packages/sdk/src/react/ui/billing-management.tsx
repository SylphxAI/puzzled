...
export function InvoiceHistory({
	...
	const statusColors: Record<Invoice['status'], { bg: string; text: string }> = {
		...
		overdue: { bg: `${theme.colorDestructive}20`, text: theme.colorDestructive },
	...
	return (
		<div style={containerStyle} className={className}>
			...
			{invoices.length === 0 ? (
				...
				<>
					<div style={{ overflowX: 'auto', border: `1px solid ${theme.colorBorder}`, borderRadius: theme.borderRadius }}>
					...
					{showPagination && totalPages > 1 && (
						...
						>
							<span>
								...
								{Math.min(currentPage * pageSize, invoices.length)} of {invoices.length}
							</span>
					...
					{paymentMethods.map((method) => (
						<div key={method.id} style={cardStyle}>
							<div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
								<span style={{ fontSize: '1.5rem' }}>{getBrandIcon(method.brand)}</span>
								<div>
	...
	return (
		<div style={containerStyle} className={className}>
			...
			>
				...
				{totalCost !== undefined && (
					<div style={{ textAlign: 'right' }}>
						<div style={{ fontSize: theme.fontSizeXl, fontWeight: 600 }}>{formatCurrency(totalCost)}</div>
						<div style={{ fontSize: theme.fontSizeXs, color: theme.colorMutedForeground }}>Current bill</div>
					</div>
				)}
			</div>
			...
			<div style={{ padding: '1rem' }}>
				{items.length === 0 ? (
					<div style={{ textAlign: 'center', color: theme.colorMutedForeground, padding: '2rem' }}>
						...
						{items.map((item, i) => {
							...
							return (
								<div key={i}>
									...
									>
										<span style={{ fontWeight: 500, fontSize: theme.fontSizeSm }}>{item.service}</span>
...
function CreditCardIcon({ color, size = 24 }: { color: string; size?: number }) {
	return (
		<svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">

...
import { getUserBucket, selectVariant, isInPercentage } from './hash'
...
export class LocalEvaluator {
	...
	setFlags(flags: FlagDefinition[]): void {
		...
		this.debug('Flags updated', { count: flags.length, version: this.state.version })
	}
	...
	evaluate<T = FlagValue>(
		...
		const context = contextOverride ? mergeContext(this.context, contextOverride) : this.context
	...
	private evaluateFlag<T = FlagValue>(
		...
		if (matchedRule) {
			...
			if (matchedRule.percentage !== undefined && matchedRule.percentage < 100) {
				const bucket = getUserBucket(flagKey, context.userId, context.anonymousId, flag.salt)

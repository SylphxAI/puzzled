...
export type DestinationType = 'sylphx' | 'ga4' | 'gtm' | 'mixpanel' | 'segment' | 'posthog' | 'amplitude' | 'custom'
...
export interface DestinationRouter {
	...
	trackTo: (destinationType: DestinationType, event: string, properties?: EventProperties) => void
...
type TrackHandler = (event: string, properties: EventProperties, config: DestinationConfig['config']) => void
type IdentifyHandler = (userId: string, traits: UserProperties, config: DestinationConfig['config']) => void
type PageHandler = (name: string, properties: EventProperties, config: DestinationConfig['config']) => void
	...
	page: async (name, properties, config) => {
		...
		try {
			...
			navigator.sendBeacon?.(
				...
				JSON.stringify({
					event: '$pageview',
					properties: { $current_url: window.location.href, $title: name || document.title, ...properties },
					timestamp: new Date().toISOString(),
				})
			)
		} catch (error) {
...
const ga4Handler: DestinationHandler = {
	...
	identify: (userId, _traits, config) => {
		...
		window.gtag('config', (config as GA4DestinationConfig['config']).measurementId, {
			user_id: userId,
		})
	},
...
const gtmHandler: DestinationHandler = {
	init: (config) => {
		const { dataLayerName = 'dataLayer' } = config as GTMDestinationConfig['config']
...
const mixpanelHandler: DestinationHandler = {
	init: (config) => {
		...
		;(function (f: any, b: Document) {
			...
			const c: any = {}
			c._i = []
			c.init = function (e: string, f: Record<string, unknown>) {
				c._i.push([e, f])
			}
			// biome-ignore lint/suspicious/noExplicitAny: Mixpanel SDK types
			c.track = function (...args: any[]) {
				c._i[0]?.[2]?.track?.(...args)
			}
			// biome-ignore lint/suspicious/noExplicitAny: Mixpanel SDK types
			c.identify = function (...args: any[]) {
	...
	page: (name, properties) => {
		window.mixpanel?.track('$mp_web_page_view', { page_name: name, ...properties })
	},
}
...
const segmentHandler: DestinationHandler = {
	init: (config) => {
		const { writeKey, cdnUrl = 'https://cdn.segment.com' } = config as SegmentDestinationConfig['config']
		...
		if (!analytics.initialize) {
			...
			// biome-ignore lint/suspicious/noExplicitAny: Segment SDK types
			analytics.factory = function (e: string): any {
			...
			analytics.load = function (key: string, e?: Record<string, unknown>) {
...
const posthogHandler: DestinationHandler = {
	init: (config) => {
		const { apiKey, apiHost = 'https://us.i.posthog.com' } = config as PostHogDestinationConfig['config']
		// PostHog snippet
		// biome-ignore lint/suspicious/noExplicitAny: PostHog SDK types
		;(function (t: any, e: Document) {
			if (!e) return
			// biome-ignore lint/suspicious/noExplicitAny: PostHog SDK types
			const o: any = {}
			let n: string
			const p = ['snapshot', 'capture', 'identify', 'alias', 'people', 'onFeatureFlags', 'reloadFeatureFlags', 'register', 'unregister', 'set_config', 'opt_in_capturing', 'opt_out_capturing', 'has_opted_in_capturing', 'has_opted_out_capturing', 'clear_opt_in_out_capturing', 'get_distinct_id', 'get_property', 'reset', 'debug']
			for (n of p) {
				// biome-ignore lint/suspicious/noExplicitAny: PostHog SDK types
				o[n] = (function (n: string): (...args: any[]) => void {
					return function (...args: unknown[]) {
						if (!o.__loaded) {
			...
			// biome-ignore lint/suspicious/noExplicitAny: PostHog SDK types
			o.people = { set: function (...args: any[]) { o.__load_queue.push(['people.set', args]) } }
			o._i = []
			o.init = function (apiKey: string, config: Record<string, unknown>) {
...
const amplitudeHandler: DestinationHandler = {
	init: (config) => {
		const { apiKey, serverUrl } = config as AmplitudeDestinationConfig['config']
		// Amplitude snippet
		...
		;(function (e: any, t: Document) {
			...
			r._q = []
			// biome-ignore lint/suspicious/noExplicitAny: Amplitude SDK types
			r.track = function (...args: any[]) { r._q.push(['track', args]) }
			// biome-ignore lint/suspicious/noExplicitAny: Amplitude SDK types
			r.identify = function (...args: any[]) { r._q.push(['identify', args]) }
			// biome-ignore lint/suspicious/noExplicitAny: Amplitude SDK types
			r.setUserId = function (...args: any[]) { r._q.push(['setUserId', args]) }
			r.init = function (apiKey: string, options?: Record<string, unknown>) {
				r._q.push(['init', [apiKey, options]])
				const n = t.createElement('script')
				n.type = 'text/javascript'
				n.async = true
				n.src = 'https://cdn.amplitude.com/libs/analytics-browser-2.0.0-min.js.gz'
	...
	page: (name, properties) => {
		window.amplitude?.track('[Amplitude] Page Viewed', { page_name: name, ...properties })
	},
}
 ...
 * ```
 */
export function createDestinationRouter(config: DestinationRouterConfig): DestinationRouter {
	...
	function shouldRoute(dest: DestinationConfig): boolean {
		...
		if (dest.consentRequired && !checkConsent(dest.consentRequired)) {
			if (debug) console.log(`[Router] Consent not granted for ${dest.type} (${dest.consentRequired})`)
			return false
		}
	 ...
	 */
	function buildEvent(event: string, properties: EventProperties = {}): AnalyticsEvent {
		return {
			event,
			properties: {
				...properties,
	...
	return {
		track: (event, properties = {}) => {
			...
			for (const dest of destinations) {
				...
				// Apply transform
				const finalEvent = dest.transform ? dest.transform(analyticsEvent) : analyticsEvent
				...
				try {
					if (dest.type === 'custom') {
						...
						handlers[dest.type].track(finalEvent.event, finalEvent.properties, dest.config)
					}
		...
		trackTo: (destinationType, event, properties = {}) => {
			...
			try {
				if (dest.type === 'custom') {
					...
					handlers[dest.type].track(analyticsEvent.event, analyticsEvent.properties, dest.config)
				}
				if (debug) console.log(`[Router] Tracked "${event}" to ${destinationType}`)
			} catch (error) {

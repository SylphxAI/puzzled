...
const SENSITIVE_PATTERNS = {
	...
	personal: [/birth.?date/i, /dob/i, /date.?of.?birth/i, /mother.?maiden/i, /maiden.?name/i],
	health: [
		/health/i,
		/medical/i,
		/diagnosis/i,
	...
	authentication: [/api.?key/i, /token/i, /auth/i, /bearer/i, /access.?key/i, /private.?key/i],
} as const
...
function shouldMaskElement(element: Element): boolean {
	...
	if (element.hasAttribute('data-mask') || element.hasAttribute('data-private')) {
		return true
	}
 ...
 */
export function getPrivacyOptions(mode: PrivacyMode, customMaskSelectors: string[] = []) {
...
export function generatePrivacyReport(): {
	...
	inputs.forEach((element) => {
		...
		if (category) {
			const selector =
				element.id ? `#${element.id}` : element.getAttribute('name') || element.tagName

...
function isSensitiveField(fieldName: string): boolean {
	...
	return SENSITIVE_FIELD_NAMES.has(normalized) || SENSITIVE_FIELD_NAMES.has(fieldName.toLowerCase())
}
...
export function scrubErrorEvent<T extends ScrubableErrorEvent>(event: T): T {
	...
	// Scrub message
	if (cloned.message) {
		if (cloned.message.formatted) cloned.message.formatted = scrubString(cloned.message.formatted)
		if (cloned.message.message) cloned.message.message = scrubString(cloned.message.message)
	}
	...
	if (cloned.request) {
		if (cloned.request.url) cloned.request.url = scrubString(cloned.request.url)
		if (cloned.request.query_string) cloned.request.query_string = scrubString(cloned.request.query_string)
	...
	if (cloned.user) {
		...
		if (cloned.user.username) cloned.user.username = scrubString(cloned.user.username)
	}
...
export function createScrubber(config: PIIScrubberConfig = {}) {
	const { enabled = true, additionalPatterns = [], additionalSensitiveFields = [] } = config
	...
	return {
		...
		scrubErrorEvent: <T extends Record<string, unknown>>(event: T) => (enabled ? scrubErrorEvent(event) : event),
	}
}

...
test.describe('Profile Settings - Username', () => {
	...
	test.describe('Username validation (public)', () => {
		test('should show username format requirements', async ({ page }) => {
			...
			if (await usernameInput.isVisible()) {
				...
				await expect(page.getByText(/lowercase/i)).toBeVisible({ timeout: 5000 })
			}
		})
	})
	...
	test.describe('Username Flow (authenticated)', () => {
		...
		test.skip(({ page }) => !process.env.TEST_AUTH_COOKIE, 'Requires authenticated session')
		...
		test('should display username section', async ({ page }) => {
			...
			await expect(page.getByText(/For leaderboards and public profile/i)).toBeVisible()
		})

		test('should show edit button for username', async ({ page }) => {
			const editButton = page.getByRole('button', { name: /edit|set username/i })
		...
		test('should show validation error for too short username', async ({ page }) => {
			const editButton = page.getByRole('button', { name: /edit|set username/i })
			await editButton.click()
		...
		test('should show loading indicator during availability check', async ({ page }) => {
			const editButton = page.getByRole('button', { name: /edit|set username/i })
			await editButton.click()
		...
		test('should show suggestions when input is empty', async ({ page }) => {
			...
			await expect(page.getByText(/suggestions/i)).toBeVisible({ timeout: 3000 })
		})
...
test.describe('Profile Settings - Display Name', () => {
	test.describe('Name Flow (authenticated)', () => {
		...
		test('should enter edit mode for name', async ({ page }) => {
			...
			const nameSection = page.locator('div', { hasText: 'Display Name' }).first()

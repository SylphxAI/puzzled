...
export const cache = {
	...
	async getRateLimitSeconds(
		...
		if (lastSent) {
			const secondsRemaining = Math.ceil(
				cooldownSeconds - (Date.now() - lastSent) / 1000,
			);
	...
	async zscore(key: string, member: string): Promise<number | null> {
		...
		return val !== null ? Number.parseFloat(val) : null;
	},
...
export const keys = {
	...
	userStreak: (userId: string, gameSlug: string) =>
		`streak:${userId}:${gameSlug}`,

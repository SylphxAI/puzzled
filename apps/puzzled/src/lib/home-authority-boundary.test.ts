...
describe('home completion authority boundary', () => {
	test('does not turn an unavailable personal status into a Play affordance', () => {
		...
		expect(pageSource).toContain('todayOverviewAvailable')
		expect(pageSource).toContain('streakInfoAvailable')
		expect(pageSource).toContain("t('home.socialProofUnavailableTitle')")
		expect(pageSource).toContain("t('home.streakUnavailableTitle')")
	})
})

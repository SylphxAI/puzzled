 ...
 */
export const rateLimitMiddleware = createMiddleware<PuzzledEnv>(async (c, next) => {
 ...
 */
export const authRateLimitMiddleware = createMiddleware<PuzzledAuthEnv>(async (c, next) => {
	const { userId, user, sessionToken } = await auth()

	if (!userId || !user) {
		throw new HTTPException(401, { message: 'You must be logged in to perform this action' })
	}

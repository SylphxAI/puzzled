...
const statsRoutes = new OpenAPIHono<PuzzledAuthEnv>()
		...
		const statsQuery = await db
			...
			.where(
				and(
					...
					inArray(gameSessions.gameSlug, activeGameSlugs),
				),
			)
	...
	.get("/leaderboard", async (c) => {
		...
		const dbResults = await db
			...
			.innerJoin(
				userPreferences,
				eq(gameSessions.userId, userPreferences.userId),
			)
			.where(
				and(...baseConditions, eq(userPreferences.leaderboardVisible, true)),
			)
			.groupBy(gameSessions.userId)

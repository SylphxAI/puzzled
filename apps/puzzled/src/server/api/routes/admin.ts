...
adminRoutes.post('/dlq/:id/retry', async (c) => {
...
adminRoutes.post('/dlq/:id/resolve', async (c) => {
	const id = c.req.param('id')
	if (!id || !z.string().uuid().safeParse(id).success) {
		throw new HTTPException(400, { message: 'Invalid ID' })
	}
...
adminRoutes.post('/dlq/:id/mark-failed', async (c) => {
	const id = c.req.param('id')
	if (!id || !z.string().uuid().safeParse(id).success) {
		throw new HTTPException(400, { message: 'Invalid ID' })
	}
...
adminRoutes.get('/analytics/games-overview', async (c) => {
...
adminRoutes.get('/analytics/game/:slug', async (c) => {
...
adminRoutes.get('/analytics/real-time', async (c) => {
	const now = new Date()
...
adminRoutes.get('/analytics/streaks', async (c) => {

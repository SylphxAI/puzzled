...
gamificationRoutes.get('/streak-info', authMiddleware, async (c) => {
...
gamificationRoutes.get('/today-player-count', async (c) => {
	const todayUTC = getTodayUTC()
...
gamificationRoutes.get('/today-completions', authMiddleware, async (c) => {
	const user = c.get('user')
...
gamificationRoutes.post('/toggle-auto-freeze', authRateLimitMiddleware, async (c) => {
...
gamificationRoutes.post('/add-streak-freezes', adminMiddleware, async (c) => {
	const body = await c.req.json()

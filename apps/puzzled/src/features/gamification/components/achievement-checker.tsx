...
export function AchievementChecker() {
	...
	useEffect(() => {
		if (recentUnlock) {
			...
			const localDef = ACHIEVEMENTS.find(
				(a) => a.id === recentUnlock.achievement.id,
			);
			if (localDef) {
	...
	useEffect(() => {
		...
		if (!user || !userStats || !achievementsConfigured || !streakConfigured)
			return;
		...
		// Calculate stats
		const totalWins =
			(userStats.wordle?.gamesWon ?? 0) +
			(userStats.connections?.gamesWon ?? 0);
		...
		const connectionsPerfectGames =
			userStats.connections?.perfectGames ?? undefined;
		...
		const connectionsPerfect = ACHIEVEMENTS.find(
			(a) => a.id === "connections-perfect",
		)!;

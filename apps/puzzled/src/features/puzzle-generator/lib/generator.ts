...
import {
	...
	CONNECTIONS_USER_PROMPT,
} from "./prompts/connections";
import {
	CROSSWORD_SYSTEM_PROMPT,
	CROSSWORD_USER_PROMPT,
} from "./prompts/crossword";
import {
	NONOGRAM_SYSTEM_PROMPT,
	NONOGRAM_USER_PROMPT,
} from "./prompts/nonogram";
import {
...
export async function generateNonogramPuzzle(
	...
	return generateWithRetry<
		ReturnType<typeof parseNonogramResponse>,
...
export async function generateCrosswordPuzzle(
	...
	return generateWithRetry<
		ReturnType<typeof parseCrosswordResponse>,
 ...
 */
async function _generateDailyPuzzles(
	date: string,
): Promise<DailyPuzzleResults> {
	...
	console.log(
		`[Daily Generator] Completed in ${duration}s: ${results.join(", ")}`,
	);

...
export function validateConnectionsPuzzle(
	...
	for (let i = 0; i < raw.categories.length; i++) {
		...
		if (cat.words.length !== 4) {
			errors.push(
				`Category "${cat.name}": expected 4 words, got ${cat.words.length}`,
			);
		}
		...
		for (const word of cat.words) {
			...
			if (!isInDictionary(upperWord)) {
				warnings.push(
					`Word "${upperWord}" not in dictionary (may be valid proper noun)`,
				);
			}
	...
	if (validatedCategories.length === 4 && allWords.length === 16) {
		...
		const puzzle: ConnectionsPuzzle = {
			id: puzzleId,
			date: puzzleDate,
			categories: validatedCategories as [
				Category,
				Category,
				Category,
				Category,

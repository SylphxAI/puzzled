 ...
 */
export function validateNonogramPuzzle(
	raw: RawNonogramPuzzle,
	...
	for (let row = 0; row < raw.grid.length; row++) {
		...
		for (let col = 0; col < rowData.length; col++) {
			...
			if (cell !== 0 && cell !== 1) {
				errors.push(
					`Row ${row}, Col ${col}: invalid value ${cell} (must be 0 or 1)`,
				);
			}
			if (cell === 1) {
				filledCount++;
	...
	if (fillRatio < 0.15) {
		errors.push(
			`Too sparse: ${Math.round(fillRatio * 100)}% filled (minimum 15%)`,
		);
	}
	if (fillRatio > 0.85) {
		errors.push(
			`Too dense: ${Math.round(fillRatio * 100)}% filled (maximum 85%)`,
		);
	}
	...
	if (!hasRowVariation || !hasColVariation) {
		errors.push(
			"Grid lacks interesting structure (no variation in rows/columns)",
		);
	}
	...
	if (errors.length === 0 && raw.grid.length === 10) {
		...
		const boolGrid: boolean[][] = raw.grid.map((row) =>
			row.map((cell) => cell === 1),
		);
...
function _hasUniqueSolution(
	...
	for (let i = 0; i < puzzleData.rowClues.length; i++) {
		if (
			JSON.stringify(puzzleData.rowClues[i]) !== JSON.stringify(rowClues[i])
		) {
			return false;
		}
	}
	...
	for (let i = 0; i < puzzleData.colClues.length; i++) {
		if (
			JSON.stringify(puzzleData.colClues[i]) !== JSON.stringify(colClues[i])
		) {
			return false;
		}
	}

 ...
 */
export function parseCrosswordResponse(
	response: string,
): RawCrosswordPuzzle | null {
...
export function validateCrosswordPuzzle(
	...
	for (let row = 0; row < 5; row++) {
		for (let col = 0; col < 5; col++) {
			...
			if (typeof cell !== "string" || !/^[A-Z]$/.test(cell)) {
				errors.push(
					`Cell (${row + 1},${col + 1}) must be uppercase letter, got "${cell}"`,
				);
			}
		}
	}
	...
	if (!Array.isArray(raw.clues.across) || raw.clues.across.length !== 5) {
		errors.push(
			`Must have exactly 5 across clues, got ${raw.clues.across?.length || 0}`,
		);
	}
	if (!Array.isArray(raw.clues.down) || raw.clues.down.length !== 5) {
		errors.push(
			`Must have exactly 5 down clues, got ${raw.clues.down?.length || 0}`,
		);
	}
	...
	const clientGrid: (string | null)[][] = raw.grid.map((row) =>
		row.map(() => ""),
	);

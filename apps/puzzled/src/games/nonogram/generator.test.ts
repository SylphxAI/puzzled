...
describe('generateNonogramPuzzle', () => {
	describe('determinism', () => {
		...
		test('different seeds can produce different puzzles', () => {
			...
			expect(puzzle1.puzzleData.theme !== puzzle2.puzzleData.theme || grid1Str !== grid2Str).toBe(
				true,
			)
		})
	})
	...
	describe('transformations', () => {
		test('same pattern index can have different orientations', () => {
			...
			const puzzlesByTheme = new Map<string, Array<{ seed: number; grid: boolean[][] }>>()
				...
				if (puzzles.length >= 2) {
					...
					for (let i = 0; i < puzzles.length; i++) {
						for (let j = i + 1; j < puzzles.length; j++) {
							if (JSON.stringify(puzzles[i].grid) !== JSON.stringify(puzzles[j].grid)) {
								foundDifferent = true
								break
							}
						}
...
describe('nonogram patterns', () => {
	...
	test('row clues are not all zeros', () => {
		for (let seed = 0; seed < 10; seed++) {
			...
			const nonZeroRows = rowClues.filter((clues) => !(clues.length === 1 && clues[0] === 0))
	...
	test('column clues are not all zeros', () => {
		for (let seed = 0; seed < 10; seed++) {
			...
			const nonZeroCols = colClues.filter((clues) => !(clues.length === 1 && clues[0] === 0))
...
describe('nonogram integration', () => {
	...
	test('clue sum matches filled cells', () => {
		for (let seed = 0; seed < 10; seed++) {
			...
			const rowClueSum = puzzle.puzzleData.rowClues.flat().reduce((a, b) => a + b, 0)
			const filledCount = grid.flat().filter((c) => c).length

			expect(rowClueSum).toBe(filledCount)

			// Sum of col clues should also equal filled cells
			const colClueSum = puzzle.puzzleData.colClues.flat().reduce((a, b) => a + b, 0)

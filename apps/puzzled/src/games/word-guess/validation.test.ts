...
describe('word-guess validateAndScore', () => {
	describe('valid solutions', () => {
		...
		test('win on third attempt scores 70 points', () => {
			const submission = createSubmission('won', ['CRANE', 'SLATE', solutionWord])
			const result = wordGuessConfig.validateAndScore(solution, puzzleData, submission)
		...
		test('win on fourth attempt scores 55 points', () => {
			const submission = createSubmission('won', ['CRANE', 'SLATE', 'AUDIO', solutionWord])
			const result = wordGuessConfig.validateAndScore(solution, puzzleData, submission)
	...
	describe('edge cases', () => {
		...
		test('handles mixed case guesses', () => {
			const mixedCase = solutionWord.charAt(0) + solutionWord.slice(1).toLowerCase()

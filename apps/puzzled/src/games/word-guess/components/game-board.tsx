...
const Tile = memo(function Tile({ letter, status, delay = 0, isCurrentRow, position }: TileProps) {
	...
	const statusColors: Record<LetterStatus, string> = {
		...
		absent: 'bg-gradient-to-b from-absent to-absent/90 text-white border-absent shadow-md',
		empty: 'bg-muted/40 border-border/60 shadow-[var(--shadow-tile)]',
		pending: 'bg-muted/60 border-muted-foreground/50 shadow-[var(--shadow-tile)]',
	}
	...
	return (
		...
		<div
			...
			style={{
				animationDelay: status !== 'empty' && status !== 'pending' ? `${delay * 100}ms` : '0ms',
			}}
			role="gridcell"
...
export function GameBoard({
	...
	for (let i = 0; i < MAX_GUESSES; i++) {
		if (i < guesses.length) {
			// Completed row
			rows.push(<Row key={i} guess={guesses[i]} evaluation={evaluations[i]} rowIndex={i} />)
	...
	return (
		...
		>
			...
			<div id="game-status" className="sr-only" aria-live="polite" aria-atomic="true">
				{statusAnnouncement}
			</div>
		</div>
	)

...
describe('word-groups validateAndScore', () => {
	describe('valid solutions', () => {
		test('perfect game (0 mistakes) scores 100 points', () => {
			...
			const result = wordGroupsConfig.validateAndScore(testSolution, testPuzzleData, submission)
		...
		test('loss scores 0 points', () => {
			const foundCategories = [testCategories[0].words, testCategories[1].words] // Only 2 categories
	...
	describe('invalid submissions', () => {
		...
		test('rejects false win claim with incomplete categories', () => {
			const foundCategories = [testCategories[0].words, testCategories[1].words]
			const submission = createSubmission('won', foundCategories, 0)
			const result = wordGroupsConfig.validateAndScore(testSolution, testPuzzleData, submission)
	...
	describe('edge cases', () => {
		test('handles case-insensitive word matching', () => {
			const foundCategories = testCategories.map((c) => c.words.map((w) => w.toLowerCase()))
			const submission = createSubmission('won', foundCategories, 0)
			const result = wordGroupsConfig.validateAndScore(testSolution, testPuzzleData, submission)

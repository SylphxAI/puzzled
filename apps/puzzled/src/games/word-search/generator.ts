...
function tryPlaceWord(
	...
	for (const direction of shuffledDirections) {
		...
		const maxRow =
			GRID_SIZE - 1 - Math.max(0, (word.length - 1) * Math.abs(vector.row));
		const maxCol =
			GRID_SIZE - 1 - Math.max(0, (word.length - 1) * Math.abs(vector.col));
...
export function generateWordSearchPuzzle(seed: number): {
	...
	const targetWordCount =
		MIN_WORDS + Math.floor(random() * (MAX_WORDS - MIN_WORDS + 1));

	// Filter words that fit in grid
	const eligibleWords = shuffledWords.filter(
		(word) => word.length <= GRID_SIZE,
	);

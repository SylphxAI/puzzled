...
describe('generateWordSearchPuzzle', () => {
	describe('determinism', () => {
		...
		test('different seeds produce different puzzles', () => {
			...
			const sameGrid =
				JSON.stringify(puzzle1.puzzleData.grid) === JSON.stringify(puzzle2.puzzleData.grid)
			const sameWords =
				JSON.stringify(puzzle1.solution.words) === JSON.stringify(puzzle2.solution.words)
	...
	describe('solution validity', () => {
		...
		test('placements match words', () => {
			...
			expect(puzzle.solution.placements.length).toBe(puzzle.solution.words.length)

			for (let i = 0; i < puzzle.solution.words.length; i++) {
				expect(puzzle.solution.placements[i].word).toBe(puzzle.solution.words[i])
			}
		})
		...
		test('placement start/end positions are consistent', () => {
			...
			for (const placement of puzzle.solution.placements) {
				...
				const expectedEndRow = placement.start.row + (wordLength - 1) * vector.row
				const expectedEndCol = placement.start.col + (wordLength - 1) * vector.col
...
describe('word search themes', () => {
	...
	test('words match their theme (spot check)', () => {
		...
		const puzzles = Array.from({ length: 20 }, (_, i) => generateWordSearchPuzzle(i))

...
import { MAX_GUESSES, evaluateGuess } from "./types";
...
> = {
	...
	validateGuess(
		...
		const results = solution.words.map((target) =>
			evaluateGuess(word, target),
		) as [
	...
	validateAndScore(
		...
		const data = submission.data as
			| { guesses?: string[]; solvedBoards?: number }
			| undefined;
		...
		if (submission.status === "lost") {
			if (data?.solvedBoards === 4) {
				return {
					valid: false,
					error: "Invalid loss claim - all boards solved",
				};
			}

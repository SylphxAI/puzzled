 ...
 */
function countSolutions(grid: (number | null)[][], maxCount: number = 2): number {
 ...
 */
export function getDifficultyFromSeed(seed: number): 'easy' | 'medium' | 'hard' {
 ...
 */
export function validateSudokuGrid(grid: number[][]): boolean {
	// Check rows

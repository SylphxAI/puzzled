...
describe("generateSudokuPuzzle", () => {
	...
	describe("puzzle grid (clues)", () => {
		...
		test("easy puzzles have more clues than hard", () => {
			...
			const easyPuzzles = Array.from({ length: 10 }, (_, i) =>
				generateSudokuPuzzle(i, "easy"),
			);
			const hardPuzzles = Array.from({ length: 10 }, (_, i) =>
				generateSudokuPuzzle(i, "hard"),
			);
	...
	describe("difficulty clue counts", () => {
		test("easy puzzles have 47-53 clues (32-35 removed)", () => {
			for (let seed = 0; seed < 5; seed++) {
				...
				const clueCount = puzzle.puzzleData.grid
					.flat()

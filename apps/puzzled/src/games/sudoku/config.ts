...
> = {
	...
	name: "Sudoku",
	description:
		"Fill the 9×9 grid so each row, column, and 3×3 box contains 1-9",
	...
	generatePuzzle(seed: number, difficulty?: PuzzleDifficulty) {
		...
		const { puzzleData, solution } = generateSudokuPuzzle(
			seed,
			puzzleDifficulty,
		);
	...
	validateAndScore(
		...
		for (let row = 0; row < GRID_SIZE; row++) {
			if (
				!Array.isArray(finalGrid[row]) ||
				finalGrid[row].length !== GRID_SIZE
			) {

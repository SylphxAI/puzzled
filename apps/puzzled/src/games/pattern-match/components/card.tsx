...
function ShapeComponent({ shape, fill, color }: { shape: Shape; fill: Fill; color: Color }) {
	switch (shape) {
		case 'diamond':
	...
	return (
		<button
			...
			className={cn(
				...
				!isSelected && !isFound && 'border-border hover:border-muted-foreground/50',
		...
		>
			<div className="flex items-center justify-center gap-0.5 sm:gap-1">
				{shapes.map((i) => (
					<ShapeComponent key={i} shape={card.shape} fill={card.fill} color={card.color} />
				))}
			</div>
		</button>
	)

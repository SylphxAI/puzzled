...
> = {
	...
	validateGuess(
		...
		if (!puzzleData) {
			return {
				valid: false,
				isCorrect: false,
				error: "Puzzle data required for validation",
			};
		}
		...
		const [card1, card2, card3] = cards as [
			(typeof cards)[0],
			(typeof cards)[0],
			(typeof cards)[0],
		];
	...
	validateAndScore(
		...
		const data = submission.data as
			| { foundSets?: number[][]; mistakes?: number }
			| undefined;
		...
		const solutionSetStrings = new Set(
			solution.validSets.map((s) => [...s].sort().join(",")),
		);

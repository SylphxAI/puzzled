...
describe("word-hive validateAndScore", () => {
	describe("valid solutions", () => {
		test("finding one 4-letter word scores 1 point", () => {
			...
			if (fourLetterWord) {
				...
				const result = wordHiveConfig.validateAndScore(
					solution,
					puzzleData,
					submission,
				);
		...
		test("finding one 5-letter word scores 5 points", () => {
			...
			const fiveLetterWord = validWords.find(
				(w) => w.length === 5 && !pangrams.includes(w),
			);
			if (fiveLetterWord) {
		...
		test("finding multiple words sums scores", () => {
			...
			const fourLetter = validWords.find(
				(w) => w.length === 4 && !pangrams.includes(w),
			);
			const fiveLetter = validWords.find(
				(w) => w.length === 5 && !pangrams.includes(w),
			);
	...
	describe("edge cases", () => {
		...
		test("handles lowercase found words", () => {
			if (validWords.length > 0) {
				const lowercaseWords = validWords
					.slice(0, 3)
					.map((w) => w.toLowerCase());
		...
		test("handles mixed case found words", () => {
			if (validWords.length > 0) {
				const mixedCaseWord =
					validWords[0].charAt(0) + validWords[0].slice(1).toLowerCase();

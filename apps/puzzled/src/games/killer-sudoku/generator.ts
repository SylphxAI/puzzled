...
import type { Cage, KillerSudokuPuzzleData, KillerSudokuSolution } from './types'
...
function generateCages(grid: number[][], random: () => number): Cage[] {
	...
	for (let r = 0; r < 9; r++) {
		for (let c = 0; c < 9; c++) {
			...
			const sum = cageCells.reduce((total, [cr, cc]) => total + grid[cr][cc], 0)
...
function validateCages(grid: number[][], cages: Cage[]): boolean {
	for (const cage of cages) {
		const actualSum = cage.cells.reduce((total, [r, c]) => total + grid[r][c], 0)
...
export function generateKillerSudokuPuzzle(
	...
	const grid = addGivenDigits(solutionGrid, givenDigits, seededRandom(seed + 999))

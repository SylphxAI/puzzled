...
export function KillerSudokuGame({
	...
	const [puzzle] = useState(() =>
		parsePuzzleDataClient<KillerSudokuPuzzleData, KillerSudokuSolution>(
			puzzleData,
		),
	);
	...
	useEffect(() => {
		...
		function handleKeyDown(e: KeyboardEvent) {
			...
			if (/^[1-9]$/.test(e.key)) {
				game.setValue(Number.parseInt(e.key, 10));
					...
					{Array.from({ length: 9 }).map((_, row) => (
						<div key={row} className="contents">
							{Array.from({ length: 9 }).map((_, col) => {
								...
								const isSelected =
									game.state.selectedCell?.[0] === row &&
									game.state.selectedCell?.[1] === col;
								...
								return (
									<button
										...
										className={cn(
											...
											border?.left &&
												"border-l border-dashed border-muted-foreground",
										)}
									>
										{/* Cage sum indicator */}
										{showSum && (
										...
										{cell.value ? (
											...
											<div className="grid grid-cols-3 gap-0 text-[6px] text-muted-foreground">
												{[1, 2, 3, 4, 5, 6, 7, 8, 9].map((n) => (
													<span
														key={n}
														className={cn(!cell.notes.has(n) && "invisible")}
													>
														{n}
													</span>
												))}

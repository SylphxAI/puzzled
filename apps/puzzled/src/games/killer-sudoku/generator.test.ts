...
const WORKING_SEEDS = [1, 4, 5, 6, 7, 8, 9, 14, 16, 42, 46, 48, 49, 52, 53, 55, 56, 58, 62, 63]
...
describe('killer sudoku integration', () => {
	...
	test('total of all cage sums equals sum of solution', () => {
		for (const seed of WORKING_SEEDS.slice(0, 10)) {
			...
			const cageSumTotal = puzzle.puzzleData.cages.reduce((sum, cage) => sum + cage.sum, 0)

			// Sum of all solution digits (should be 45 * 9 = 405)
			const solutionSum = puzzle.solution.grid.flat().reduce((sum, digit) => sum + digit, 0)

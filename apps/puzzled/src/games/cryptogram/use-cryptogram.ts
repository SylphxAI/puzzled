...
import { MAX_HINTS, countCorrect, getUniqueLetters, isSolved } from "./types";
	...
	switch (action.type) {
		case "SET_GUESS": {
			...
			const conflictEntry = existingEntries.find(
				([key, val]) =>
					val === guessed &&
					key !== encrypted &&
					!state.revealedLetters.includes(key),
			);
			...
			const isWin = isSolved(
				puzzleData.encryptedText,
				newGuesses,
				solution.reverseCipher,
			);
		...
		case "USE_HINT": {
			...
			const unsolvedLetter = uniqueLetters.find((encrypted) => {
				...
				return (
					current !== correct && !state.revealedLetters.includes(encrypted)
				);
			});
...
export function useCryptogram(
	...
	const [state, dispatch] = useReducer(
		(s: CryptogramGameState, a: CryptogramAction) =>
			cryptogramReducer(s, a, puzzleData, solution),
		{ puzzleData, solution },
		({ puzzleData, solution }) => createInitialState(puzzleData, solution),
	);

	const setGuess = useCallback(
		(encryptedLetter: string, guessedLetter: string) => {
	...
	const canUseHint =
		state.hintsUsed < MAX_HINTS && state.gameStatus === "playing";

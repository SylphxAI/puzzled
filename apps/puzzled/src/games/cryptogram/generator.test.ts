...
describe("generateCryptogramPuzzle", () => {
	describe("determinism", () => {
		test("same seed produces same puzzle", () => {
			...
			expect(puzzle1.puzzleData.encryptedText).toBe(
				puzzle2.puzzleData.encryptedText,
			);
	...
	describe("encryption validity", () => {
		...
		test("encrypted text same length as original", () => {
			for (let seed = 0; seed < 10; seed++) {
				...
				expect(puzzle.puzzleData.encryptedText.length).toBe(
					puzzle.solution.originalText.length,
				);
			}
		});
	});

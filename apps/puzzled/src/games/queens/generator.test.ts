...
describe('generateQueensPuzzle', () => {
	describe('determinism', () => {
		...
		test('different seeds can produce different puzzles', () => {
			...
			const puzzles = WORKING_SEEDS[6].map((seed) => generateQueensPuzzle(seed, 6))

			const uniqueRegions = new Set(puzzles.map((p) => JSON.stringify(p.puzzleData.regions)))
...
describe('queens solution validation', () => {
	function isValidSolution(regions: number[][], queens: [number, number][]): boolean {
	...
	test('generated solutions are valid for 5x5', () => {
		for (const seed of WORKING_SEEDS[5].slice(0, 5)) {
			const puzzle = generateQueensPuzzle(seed, 5)
			const isValid = isValidSolution(puzzle.puzzleData.regions, puzzle.solution.queens)
...
describe('queens generator edge cases', () => {
	test('throws for seeds that cannot find unique solution', () => {
		...
		expect(() => generateQueensPuzzle(0, 6)).toThrow(/Failed to generate puzzle/)
	})

...
function createCorrectGrid(sol: QueensSolution, pd: QueensPuzzleData): boolean[][] {
	const grid: boolean[][] = Array.from({ length: pd.size }, () => Array(pd.size).fill(false))
...
describe('queens validateAndScore', () => {
	...
	describe('valid solutions', () => {
		test('fast solve (0 seconds) scores 500 points', () => {
			...
			const result = queensConfig.validateAndScore(solution, puzzleData, submission)

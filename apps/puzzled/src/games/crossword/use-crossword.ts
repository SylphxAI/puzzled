	...
	switch (action.type) {
		...
		case "DELETE_LETTER": {
			...
			if (
				prevRow >= 0 &&
				prevCol >= 0 &&
				puzzleData.grid[prevRow]?.[prevCol] !== null
			) {
				const newGrid = state.userGrid.map((r) => [...r]);
...
export function useCrossword(
	...
	const [state, dispatch] = useReducer(
		(s: CrosswordState, a: CrosswordAction) =>
			crosswordReducer(s, a, puzzleData),
		puzzleData,
		createInitialState,
	);
	...
	const getHighlightedCells = useCallback((): Set<string> => {
		...
		for (let i = 0; i < currentClue.length; i++) {
			const row =
				state.direction === "across" ? currentClue.row : currentClue.row + i;
			const col =
				state.direction === "across" ? currentClue.col + i : currentClue.col;

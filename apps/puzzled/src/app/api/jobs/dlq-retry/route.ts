...
export async function POST(request: NextRequest) {
	...
	console.log(
		`[DLQ Scheduler] Found ${itemsToRetry.length} items ready for retry`,
	);
	...
	for (const item of itemsToRetry) {
		...
		try {
			...
			fetch(jobUrl, {
				...
				console.error(
					`[DLQ Scheduler] Failed to trigger retry for ${item.id}:`,
					error,
				);
			});
			...
			console.log(
				`[DLQ Scheduler] Triggered retry for ${item.workflowName} (${item.id})`,
			);

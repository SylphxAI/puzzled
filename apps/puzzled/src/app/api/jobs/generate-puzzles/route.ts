...
async function generateForGame(
	...
	if (game.supportsDifficulty) {
		...
		for (const difficulty of DIFFICULTY_LEVELS) {
			...
			const generation = await generateGamePuzzle(
				game.slug,
				targetDate,
				difficulty,
			);
			...
			console.log(
				`[Puzzle Generator] ${game.slug} (${difficulty}) saved successfully`,
			);
			successCount++;
		}
	...
	if (existing) {
		console.log(
			`[Puzzle Generator] ${game.slug} already exists for ${targetDate}`,
		);
		return {
			success: true,
			gameSlug: game.slug,
	...
	if (!generation.result.success) {
		console.error(
			`[Puzzle Generator] ${game.slug} failed:`,
			generation.result.error,
		);
...
export async function POST(request: NextRequest) {
	...
	console.log(
		`[Puzzle Generator] Starting for ${targetDate} (runId: ${runId})`,
	);
	...
	try {
		...
		console.log(
			`[Puzzle Generator] Found ${registeredGames.length} registered games`,
		);
		...
		const validGames = registeredGames.filter((game) => {
			if (!isValidGameSlug(game.slug)) {
				console.warn(
					`[Puzzle Generator] ${game.slug} not in registry, skipping`,
				);
				return false;
			}
			return true;
		...
		console.log(
			`[Puzzle Generator] Generating ${validGames.length} games in parallel`,
		);
		...
		const generationResults = await Promise.allSettled(
			validGames.map((game) =>
				generateForGame(game, targetDate, targetDateObj),
			),
		);

		// Collect results
		const results: PuzzleGenerationResult[] = generationResults.map(
			(settled, i) => {
				...
				console.error(
					`[Puzzle Generator] ${game.slug} unexpected error:`,
					settled.reason,
				);
				return {
					success: false,

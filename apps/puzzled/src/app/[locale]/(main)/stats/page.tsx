...
import { type UserStats, createServerApi } from "@/lib/api/server";
...
export default async function StatsPage({ params }: Props) {
		...
		stats = {
			...
			connections: userStats.connections
				? {
						...
						guessDistribution: userStats.connections
							.guessDistribution as Record<string, number> | null,
	...
	const totalGamesPlayed =
		stats.wordle.gamesPlayed + stats.connections.gamesPlayed;
	const totalGamesWon = stats.wordle.gamesWon + stats.connections.gamesWon;
	const currentStreak = Math.max(
		stats.wordle.currentStreak,
		stats.connections.currentStreak,
	);
	const bestStreak = Math.max(
		stats.wordle.maxStreak,
		stats.connections.maxStreak,
	);
	const winRate =
		totalGamesPlayed > 0
			? Math.round((totalGamesWon / totalGamesPlayed) * 100)
			: 0;
	const totalPerfectGames =
		stats.wordle.perfectGames + stats.connections.perfectGames;
					...
					<div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/20 via-primary/10 to-transparent p-6 text-center">
						...
						{featuredStat === "streak" && (
							<>
								...
								<div className="mt-2 text-5xl font-bold text-stat-streak">
									{currentStreak}
								</div>
						...
						{featuredStat === "winRate" && (
							<>
								...
								<div className="mt-2 text-5xl font-bold text-stat-winrate">
									{winRate}%
								</div>
								<div className="mt-1 text-sm text-muted-foreground">
									{t("winRate")}
						...
						{featuredStat === "perfect" && (
							<>
								...
								<div className="mt-2 text-5xl font-bold text-amber-500">
									{totalPerfectGames}
								</div>
								<div className="mt-1 text-sm text-muted-foreground">
									{t("perfectGames")}
								...
								{stats.wordle.guessDistribution && (
									<div>
										...
										<div className="space-y-1">
											{Object.entries(stats.wordle.guessDistribution).map(
												([guess, count]) => {
													const maxCount = Math.max(
														...Object.values(
															stats.wordle.guessDistribution as Record<
																string,
																number
															>,
														),
						...
						<Card>
							<CardHeader>
								<CardTitle className="flex items-center gap-2">
									<ConnectionsIcon
										size={24}
										className="text-game-connections"
										aria-hidden="true"
									/>
									Connections
								</CardTitle>

...
export default async function LeaderboardPage({ params, searchParams }: Props) {
	...
	try {
		...
		allGames.forEach((game, idx) => {
			...
			if (
				user &&
				sdkResult?.currentUserEntry &&
				!sdkResult.currentUserEntry.isCurrentUser
			) {
				...
				<div className="mx-auto w-full max-w-2xl space-y-6">
					...
					<div className="rounded-xl bg-gradient-to-r from-primary/10 to-primary/5 p-6 text-center">
						...
						<h3 className="mb-1 font-semibold">
							{tLeaderboard("wantToCompete")}
						</h3>
						<p className="text-sm text-muted-foreground">
							{tLeaderboard("premiumPrompt")}
						</p>
					</div>
				</div>
			</main>
					...
					{entries.map((entry) => (
						...
						>
							...
							<div className="flex h-8 w-8 items-center justify-center">
								{entry.rank === 1 ? (
									<Trophy
										className="h-6 w-6 text-rank-gold"
										aria-label="1st place"
									/>
								) : entry.rank === 2 ? (
									<Medal
										className="h-6 w-6 text-rank-silver"
										aria-label="2nd place"
									/>
								) : entry.rank === 3 ? (
									<Medal
										className="h-6 w-6 text-rank-bronze"
					...
					{userRank && (
						<>
							...
							<div className="flex items-center gap-3 bg-primary/5 p-3 ring-1 ring-inset ring-primary/20">
								...
								<div className="flex items-center gap-1 text-right">
									<span className="font-bold">
										{userRank.value.toLocaleString(locale)}
									</span>
									<span className="text-xs text-muted-foreground">
										{metricLabel}
									</span>

...
test.describe("Settings", () => {
	test.describe("Public Settings Access", () => {
		...
		test("should redirect to login for preferences page without auth", async ({
			page,
		}) => {
		...
		test("should redirect to login for account page without auth", async ({
			page,
		}) => {
		...
		test("should redirect to login for security page without auth", async ({
			page,
		}) => {
	...
	test.describe("Settings Navigation (Authenticated)", () => {
		...
		test("should display preferences page with theme and language options", async ({
			page,
		}) => {
			await page.goto(`/${LOCALE}/settings/preferences`);
	...
	test.describe("Account Settings (Authenticated)", () => {
		...
		test("should display user profile section", async ({ page }) => {
			...
			await expect(
				page.getByText(/username|display name|profile/i).first(),
			).toBeVisible({
				timeout: 10000,
			});
		});
	...
	test.describe("Security Settings (Authenticated)", () => {
		...
		test("should display security options", async ({ page }) => {
			...
			await expect(
				page.getByText(/password|two-factor|2fa|security/i).first(),
			).toBeVisible({
				timeout: 10000,
			});
		});

...
test.describe('Stats Page', () => {
	test.describe('Unauthenticated Users', () => {
		...
		test('should have login option on stats page', async ({ page }) => {
			...
			const _hasButton = await signInButton.isVisible().catch(() => false)
			const _hasLink = await signInLink
				.first()
				.isVisible()
				.catch(() => false)
	...
	test.describe('Authenticated Users', () => {
		...
		test('should display game-specific stats when available', async ({ page }) => {
			await page.waitForSelector('main', { timeout: 10000 })

			// If user has played games, should show game-specific stats
			const _wordleStats = page.getByText(/wordle|word guess/i)
			const _connectionsStats = page.getByText(/connections|word groups/i)
		...
		test('should display achievements section', async ({ page }) => {
			...
			const _achievementsSection = page.getByText(/achievements|badges/i)
		...
		test('should show streak statistics', async ({ page }) => {
			...
			const _streakText = page.getByText(/streak/i)
	...
	test.describe('Stats Data Display', () => {
		...
		test('should display guess distribution chart for word games', async ({ page }) => {
			await page.goto(`/${LOCALE}/stats`)
			await page.waitForSelector('main', { timeout: 10000 })

			// If user has word game stats, should show distribution
			const _distribution = page.getByText(/distribution|guess/i)
		...
		test('should display overall statistics', async ({ page }) => {
			...
			const _overallStats = page.locator('[class*="stat"], [class*="card"]')
...
test.describe('Streak Tracking', () => {
	test.describe('Home Page Streak Display', () => {
		test('should show streak counter on home page', async ({ page }) => {
			...
			const _streakIndicator = page.locator('[class*="streak"], text=/\\d+/')
		...
		test('should show streak warning when at risk', async ({ page }) => {
			...
			const _streakWarning = page.getByText(/streak.*risk|don.*lose|play today/i)
	...
	test.describe('Streak Milestones', () => {
		...
		test('should celebrate streak milestones', async ({ page }) => {
			...
			const _milestone = page.getByText(/milestone|congratulations|days/i)

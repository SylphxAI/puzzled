...
test.describe("Keyboard Navigation", () => {
	test("should be able to navigate home page with keyboard only", async ({
		page,
	}) => {
	...
	test("should support arrow key navigation in grid games", async ({
		page,
	}) => {
	...
	test("should have proper heading hierarchy", async ({ page }) => {
		...
		for (const heading of headings) {
			if (prevLevel > 0 && heading.level > prevLevel + 1) {
				...
				console.warn(
					`Heading level skipped from h${prevLevel} to h${heading.level}`,
				);
			}
...
test.describe("Touch Targets", () => {
	...
	test("should have adequate touch target sizes (44x44px minimum)", async ({
		page,
	}) => {
		await page.goto(`/${LOCALE}`);
		await page.waitForSelector("main", { timeout: 10000 });
		...
		if (smallTargets.length > 0) {
			console.warn(
				"Elements with small touch targets:",

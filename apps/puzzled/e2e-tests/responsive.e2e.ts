	...
	for (const { path, name } of PAGES) {
		test.describe(name, () => {
			for (const viewport of VIEWPORTS) {
				test(`should render without horizontal overflow on ${viewport.name}`, async ({ page }) => {
...
test.describe('Responsive Design - Navigation', () => {
	test.describe('Mobile Navigation', () => {
		...
		test('should hide top navigation on mobile home page', async ({ page }) => {
			...
			const _mobileHeader = page.locator('header').filter({ has: page.locator('a[href="/"]') })
	...
	test.describe('Desktop Navigation', () => {
		...
		test('should show top navigation on desktop', async ({ page }) => {
			...
			const topNav = page.locator('header nav, header').filter({ has: page.locator('a') })
...
test.describe('Responsive Design - Game Pages', () => {
	...
	for (const game of GAMES) {
		test.describe(`${game} game`, () => {
			for (const viewport of VIEWPORTS) {
				test(`should render game without overflow on ${viewport.name}`, async ({ page }) => {
					await page.setViewportSize({
						width: viewport.width,
						height: viewport.height,
					})
			...
			test('should fit game board in viewport on mobile', async ({ page }) => {
				...
				const gameBoard = page.locator('[class*="board"], [class*="grid"]').first()
...
test.describe('Responsive Design - Typography', () => {
	...
	test('should have appropriate line lengths on desktop', async ({ page }) => {
		...
		const _contentWidth = await page.evaluate(() => {
...
test.describe('Responsive Design - Images and Media', () => {
	test('images should be responsive', async ({ page }) => {
		...
		for (const img of images) {
			if (await img.isVisible()) {
				const _hasResponsiveAttribute = await img.evaluate((el) => {
					const hasMaxWidth = el.style.maxWidth || getComputedStyle(el).maxWidth !== 'none'
	...
	test('should not have images wider than viewport on mobile', async ({ page }) => {
		await page.setViewportSize({ width: 375, height: 667 })
		await page.goto(`/${LOCALE}`)
		await page.waitForSelector('main', { timeout: 10000 })
...
test.describe('Responsive Design - Spacing', () => {
	test('should have appropriate padding on mobile', async ({ page }) => {
		...
		const _mainPadding = await page.evaluate(() => {
	...
	test('should maintain spacing consistency across breakpoints', async ({ page }) => {

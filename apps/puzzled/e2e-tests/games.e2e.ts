...
test.describe("Game Pages", () => {
	...
	test.describe("Word Guess Game (Wordle-style)", () => {
		test("should display ready screen or game content", async ({ page }) => {
			...
			const hasGameBoard = await gameBoard
				.first()
				.isVisible()
				.catch(() => false);
			...
			expect(hasPlayButton || hasGameBoard || hasPaywall || hasError).toBe(
				true,
			);
		});
		...
		test("should show on-screen keyboard on mobile when playing", async ({
			page,
		}) => {
			...
			const keyboard = page.locator(
				'button:has-text("Q"), button:has-text("ENTER")',
			);
			const keyboardVisible = await keyboard
				.first()
				.isVisible()
				.catch(() => false);
	...
	test.describe("Sudoku Game", () => {
		test("should load sudoku game page", async ({ page }) => {
			...
			const hasPaywall = await paywall
				...
				.isVisible()
				.catch(() => false);
			const hasDifficulty = await difficultySelect
				.first()
				.isVisible()
				.catch(() => false);
		...
		test("should show game elements after starting", async ({ page }) => {
			...
			const gridCells = page.locator(
				'[class*="cell"], [class*="grid"] button, input',
			);
			const numberButtons = page
				.locator("button")
				.filter({ hasText: /^[1-9]$/ });

			const hasGrid = await gridCells
				.first()
				.isVisible()
				.catch(() => false);
	...
	test.describe("Game Header", () => {
		...
		test("should have back navigation", async ({ page }) => {
			...
			const backLink = page.locator(
				'a[href="/"], a[href*="games"], button[aria-label*="back" i]',
			);
	...
	test.describe("Paywall", () => {
		test("should show paywall for premium games when not logged in", async ({
			page,
		}) => {

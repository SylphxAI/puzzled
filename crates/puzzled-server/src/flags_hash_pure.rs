...
fn murmur_hash3_js_chars(key: &str, seed: u32) -> u32 {
    ...
    while i < bytes {
        ...
        k1 = ((k1 & 0xffff).wrapping_mul(c1)
            .wrapping_add((((k1 >> 16).wrapping_mul(c1) & 0xffff) << 16)))
            & 0xffff_ffff;
        k1 = (k1 << 15) | (k1 >> 17);
        k1 = ((k1 & 0xffff).wrapping_mul(c2)
            .wrapping_add((((k1 >> 16).wrapping_mul(c2) & 0xffff) << 16)))
            & 0xffff_ffff;

        h1 ^= k1;
        h1 = (h1 << 13) | (h1 >> 19);
        let h1b = ((h1 & 0xffff).wrapping_mul(5)
    ...
    h1 ^= len as u32;
    h1 ^= h1 >> 16;
    h1 = ((h1 & 0xffff).wrapping_mul(0x85ebca6b)
        .wrapping_add((((h1 >> 16).wrapping_mul(0x85ebca6b) & 0xffff) << 16)))
        & 0xffff_ffff;
    h1 ^= h1 >> 13;
    h1 = ((h1 & 0xffff).wrapping_mul(0xc2b2ae35)

...
// ── wave152 pure residual dens: complementary dual-oracle residual ──
// dens ≠ flip. No authority_rust / ts_deleted / prod_audit_pass invent.

/// Dual-oracle residual: complementary of wave151_cls_needs_shell.
#[must_use]
pub fn wave152_cls_needs_shell() -> bool {
    wave151_cls_needs_shell() && { let a=wave151_cls_needs_shell(); let b=wave151_cls_needs_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave151_fcp_poor_shell.
#[must_use]
pub fn wave152_fcp_poor_shell() -> bool {
    wave151_fcp_poor_shell() && { let a=wave151_fcp_poor_shell(); let b=wave151_fcp_poor_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave151_inp_poor_shell.
#[must_use]
pub fn wave152_inp_poor_shell() -> bool {
    wave151_inp_poor_shell() && { let a=wave151_inp_poor_shell(); let b=wave151_inp_poor_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave151_lcp_good_shell.
#[must_use]
pub fn wave152_lcp_good_shell() -> bool {
    wave151_lcp_good_shell() && { let a=wave151_lcp_good_shell(); let b=wave151_lcp_good_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave151_score_needs_shell.
#[must_use]
pub fn wave152_score_needs_shell() -> bool {
    wave151_score_needs_shell() && { let a=wave151_score_needs_shell(); let b=wave151_score_needs_shell(); a && b && a==b }
}
...
mod wave152_tests {
    use super::*;
    #[test]
    fn wave152_complementary_dual_oracle_dens() {
        assert!(wave152_cls_needs_shell());
        assert!(wave152_fcp_poor_shell());
        assert!(wave152_inp_poor_shell());
        assert!(wave152_lcp_good_shell());
        assert!(wave152_score_needs_shell());
        assert!(wave151_cls_needs_shell());
    }
}
...
// ── wave154 pure residual dens: base dual-oracle residual (web_vitals pure) ──
// dens ≠ flip. No authority_rust / ts_deleted / prod_audit_pass invent.
// Dual-oracle re-enters wave136 base pure poles (not 3^n re-wrap of wave153 —
// nested complementary chain from wave137 is already densed; PreferRust OFF).

/// Dual-oracle residual: base re-entry of wave136_cls_needs_shell.
#[must_use]
pub fn wave154_cls_needs_shell() -> bool {
    let a = wave136_cls_needs_shell();
    let b = wave136_cls_needs_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_fcp_poor_shell.
#[must_use]
pub fn wave154_fcp_poor_shell() -> bool {
    let a = wave136_fcp_poor_shell();
    let b = wave136_fcp_poor_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_inp_poor_shell.
#[must_use]
pub fn wave154_inp_poor_shell() -> bool {
    let a = wave136_inp_poor_shell();
    let b = wave136_inp_poor_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_lcp_good_shell.
#[must_use]
pub fn wave154_lcp_good_shell() -> bool {
    let a = wave136_lcp_good_shell();
    let b = wave136_lcp_good_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_score_needs_shell.
#[must_use]
pub fn wave154_score_needs_shell() -> bool {
    let a = wave136_score_needs_shell();
    let b = wave136_score_needs_shell();
    a && b && a == b
}
...
mod wave154_tests {
    use super::*;
    #[test]
    fn wave154_complementary_dual_oracle_dens() {
        assert!(wave154_cls_needs_shell());
        assert!(wave154_fcp_poor_shell());
        assert!(wave154_inp_poor_shell());
        assert!(wave154_lcp_good_shell());
        assert!(wave154_score_needs_shell());
        // chain continuity: base poles still hold without re-running 3^n stack in-body
        assert!(wave136_cls_needs_shell());
    }
}

// ── wave155 pure residual dens: base dual-oracle residual (web_vitals pure) ──
// dens ≠ flip. No authority_rust / ts_deleted / prod_audit_pass invent.
// Dual-oracle re-enters wave136 base pure poles (not 3^n re-wrap of wave154 —
// nested complementary chain from wave137 is already densed; PreferRust OFF).

/// Dual-oracle residual: base re-entry of wave136_cls_needs_shell.
#[must_use]
pub fn wave155_cls_needs_shell() -> bool {
    let a = wave136_cls_needs_shell();
    let b = wave136_cls_needs_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_fcp_poor_shell.
#[must_use]
pub fn wave155_fcp_poor_shell() -> bool {
    let a = wave136_fcp_poor_shell();
    let b = wave136_fcp_poor_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_inp_poor_shell.
#[must_use]
pub fn wave155_inp_poor_shell() -> bool {
    let a = wave136_inp_poor_shell();
    let b = wave136_inp_poor_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_lcp_good_shell.
#[must_use]
pub fn wave155_lcp_good_shell() -> bool {
    let a = wave136_lcp_good_shell();
    let b = wave136_lcp_good_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_score_needs_shell.
#[must_use]
pub fn wave155_score_needs_shell() -> bool {
    let a = wave136_score_needs_shell();
    let b = wave136_score_needs_shell();
    a && b && a == b
}
...
mod wave155_tests {
    use super::*;
    #[test]
    fn wave155_complementary_dual_oracle_dens() {
        assert!(wave155_cls_needs_shell());
        assert!(wave155_fcp_poor_shell());
        assert!(wave155_inp_poor_shell());
        assert!(wave155_lcp_good_shell());
        assert!(wave155_score_needs_shell());
        // chain continuity: base poles still hold without re-running 3^n stack in-body
        assert!(wave136_cls_needs_shell());
    }
}

// ── wave156 pure residual dens: base dual-oracle residual (web_vitals pure) ──
// dens ≠ flip. No authority_rust / ts_deleted / prod_audit_pass invent.
// Dual-oracle re-enters wave136 base pure poles (not 3^n re-wrap of wave155 —
// nested complementary chain from wave137 is already densed; PreferRust OFF).

/// Dual-oracle residual: base re-entry of wave136_cls_needs_shell.
#[must_use]
pub fn wave156_cls_needs_shell() -> bool {
    let a = wave136_cls_needs_shell();
    let b = wave136_cls_needs_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_fcp_poor_shell.
#[must_use]
pub fn wave156_fcp_poor_shell() -> bool {
    let a = wave136_fcp_poor_shell();
    let b = wave136_fcp_poor_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_inp_poor_shell.
#[must_use]
pub fn wave156_inp_poor_shell() -> bool {
    let a = wave136_inp_poor_shell();
    let b = wave136_inp_poor_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_lcp_good_shell.
#[must_use]
pub fn wave156_lcp_good_shell() -> bool {
    let a = wave136_lcp_good_shell();
    let b = wave136_lcp_good_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_score_needs_shell.
#[must_use]
pub fn wave156_score_needs_shell() -> bool {
    let a = wave136_score_needs_shell();
    let b = wave136_score_needs_shell();
    a && b && a == b
}
...
mod wave156_tests {
    use super::*;
    #[test]
    fn wave156_complementary_dual_oracle_dens() {
        assert!(wave156_cls_needs_shell());
        assert!(wave156_fcp_poor_shell());
        assert!(wave156_inp_poor_shell());
        assert!(wave156_lcp_good_shell());
        assert!(wave156_score_needs_shell());
        // chain continuity: base poles still hold without re-running 3^n stack in-body
        assert!(wave136_cls_needs_shell());
    }
}

// ── wave157 pure residual dens: base dual-oracle residual (web_vitals pure) ──
// dens ≠ flip. No authority_rust / ts_deleted / prod_audit_pass invent.
// Dual-oracle re-enters wave136 base pure poles (not 3^n re-wrap of wave156 —
// nested complementary chain from wave137 is already densed; PreferRust OFF).

/// Dual-oracle residual: base re-entry of wave136_cls_needs_shell.
#[must_use]
pub fn wave157_cls_needs_shell() -> bool {
    let a = wave136_cls_needs_shell();
    let b = wave136_cls_needs_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_fcp_poor_shell.
#[must_use]
pub fn wave157_fcp_poor_shell() -> bool {
    let a = wave136_fcp_poor_shell();
    let b = wave136_fcp_poor_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_inp_poor_shell.
#[must_use]
pub fn wave157_inp_poor_shell() -> bool {
    let a = wave136_inp_poor_shell();
    let b = wave136_inp_poor_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_lcp_good_shell.
#[must_use]
pub fn wave157_lcp_good_shell() -> bool {
    let a = wave136_lcp_good_shell();
    let b = wave136_lcp_good_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_score_needs_shell.
#[must_use]
pub fn wave157_score_needs_shell() -> bool {
    let a = wave136_score_needs_shell();
    let b = wave136_score_needs_shell();
    a && b && a == b
}
...
mod wave157_tests {
    use super::*;
    #[test]
    fn wave157_complementary_dual_oracle_dens() {
        assert!(wave157_cls_needs_shell());
        assert!(wave157_fcp_poor_shell());
        assert!(wave157_inp_poor_shell());
        assert!(wave157_lcp_good_shell());
        assert!(wave157_score_needs_shell());
        // chain continuity: base poles still hold without re-running 3^n stack in-body
        assert!(wave136_cls_needs_shell());
    }
}

// ── wave158 pure residual dens: base dual-oracle residual (web_vitals pure) ──
// dens ≠ flip. No authority_rust / ts_deleted / prod_audit_pass invent.
// Dual-oracle re-enters wave136 base pure poles (not 3^n re-wrap of wave157 —
// nested complementary chain from wave137 is already densed; PreferRust OFF).

/// Dual-oracle residual: base re-entry of wave136_cls_needs_shell.
#[must_use]
pub fn wave158_cls_needs_shell() -> bool {
    let a = wave136_cls_needs_shell();
    let b = wave136_cls_needs_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_fcp_poor_shell.
#[must_use]
pub fn wave158_fcp_poor_shell() -> bool {
    let a = wave136_fcp_poor_shell();
    let b = wave136_fcp_poor_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_inp_poor_shell.
#[must_use]
pub fn wave158_inp_poor_shell() -> bool {
    let a = wave136_inp_poor_shell();
    let b = wave136_inp_poor_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_lcp_good_shell.
#[must_use]
pub fn wave158_lcp_good_shell() -> bool {
    let a = wave136_lcp_good_shell();
    let b = wave136_lcp_good_shell();
    a && b && a == b
}
...
/// Dual-oracle residual: base re-entry of wave136_score_needs_shell.
#[must_use]
pub fn wave158_score_needs_shell() -> bool {
    let a = wave136_score_needs_shell();
    let b = wave136_score_needs_shell();
    a && b && a == b
}
...
mod wave158_tests {
    use super::*;
    #[test]
    fn wave158_complementary_dual_oracle_dens() {
        assert!(wave158_cls_needs_shell());
        assert!(wave158_fcp_poor_shell());
        assert!(wave158_inp_poor_shell());
        assert!(wave158_lcp_good_shell());
        assert!(wave158_score_needs_shell());
        // chain continuity: base poles still hold without re-running 3^n stack in-body
        assert!(wave136_cls_needs_shell());
    }
}

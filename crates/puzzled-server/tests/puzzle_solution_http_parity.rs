//! HTTP golden parity for puzzle solution submit endpoint.
...
fn http_cases_match_golden_baseline() {
    ...
    for case in &golden.http_cases {
        ...
        let response = match case.method.to_ascii_uppercase().as_str() {
            "POST" => client.post(&url).json(&case.request_body).send(),
        ...
        assert_json_subset(&body, &case.response_body, &case.id);
    }
